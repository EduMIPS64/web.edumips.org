...
` + e.stack) : e;
	...
	(function(e) {
		...
		function g(R, S, E = 100, A = !1, F = !1, B, H) {
			let Y, J, Ae, Rt = 0, Ze;
			const At = new ue({
...
`));
	...
	var $t = class et {
		...
		static {
			this.cache = new Vs({ getCacheKey: JSON.stringify }, (t) => {
				function n(c) {
	...
	var ke = class {
		...
		WALKTRACE(t, n, r, s, a, l, o, u, c, m, h, g, f, b, p, _, v, L) {
			let y = null, N = null, k = new $1(), M = n, w = r, C = f[0] - _[0] - s, T = -1073741824, U = this.m_forwardHistory.length - 1;
			do {
				const x = C + t;
			...
			if (y = k.getReverseChanges(), L[0]) {
				let x = f[0] + 1, R = _[0] + 1;
				if (y !== null && y.length > 0) {
					const S = y[y.length - 1];
					x = Math.max(x, S.getOriginalEnd()), R = Math.max(R, S.getModifiedEnd());
				}
				N = [new we(x, g - x + 1, R, p - R + 1)];
			} else {
				k = new $1(), M = l, w = o, C = f[0] - _[0] - u, T = 1073741824, U = v ? this.m_reverseHistory.length - 1 : this.m_reverseHistory.length - 2;
				do {
					const x = C + a;
			...
			return this.ConcatenateChanges(y, N);
		}
		ComputeRecursionPoint(t, n, r, s, a, l, o) {
			...
			const b = n - t + (s - r), p = b + 1, _ = new Int32Array(p), v = new Int32Array(p), L = s - r, y = n - t, N = t - r, k = n - s, M = (y - L) % 2 === 0;
			_[L] = t, v[y] = n, o[0] = !1;
			for (let w = 1; w <= b / 2 + 1; w++) {
				let C = 0, T = 0;
				m = this.ClipDiagonalBound(L - w, w, L, p), h = this.ClipDiagonalBound(L + w, w, L, p);
				for (let x = m; x <= h; x += 2) {
					x === m || x < h && _[x - 1] < _[x + 1] ? u = _[x + 1] : u = _[x - 1] + 1, c = u - (x - L) - N;
					const R = u;
					for (; u < n && c < s && this.ElementsAreEqual(u + 1, c + 1);) u++, c++;
					if (_[x] = u, u + c > C + T && (C = u, T = c), !M && Math.abs(x - y) <= w - 1 && u >= v[x]) return a[0] = u, l[0] = c, R <= v[x] && w <= 1448 ? this.WALKTRACE(L, m, h, N, y, g, f, k, _, v, u, n, a, c, s, l, M, o) : null;
				}
				const U = (C - t + (T - r) - w) / 2;
				if (this.ContinueProcessingPredicate !== null && !this.ContinueProcessingPredicate(C, U)) return o[0] = !0, a[0] = C, l[0] = T, U > 0 && w <= 1448 ? this.WALKTRACE(L, m, h, N, y, g, f, k, _, v, u, n, a, c, s, l, M, o) : (t++, r++, [new we(t, n - t + 1, r, s - r + 1)]);
				g = this.ClipDiagonalBound(y - w, w, y, p), f = this.ClipDiagonalBound(y + w, w, y, p);
				for (let x = g; x <= f; x += 2) {
					x === g || x < f && v[x - 1] >= v[x + 1] ? u = v[x + 1] - 1 : u = v[x - 1], c = u - (x - y) - k;
					const R = u;
					for (; u > t && c > r && this.ElementsAreEqual(u, c);) u--, c--;
					if (v[x] = u, M && Math.abs(x - L) <= w && u <= _[x]) return a[0] = u, l[0] = c, R >= _[x] && w <= 1448 ? this.WALKTRACE(L, m, h, N, y, g, f, k, _, v, u, n, a, c, s, l, M, o) : null;
				}
				if (w <= 1447) {
					let x = new Int32Array(h - m + 2);
					x[0] = L - m + 1, Me.Copy2(_, m, x, 1, h - m + 1), this.m_forwardHistory.push(x), x = new Int32Array(f - g + 2), x[0] = y - g + 1, Me.Copy2(v, g, x, 1, f - g + 1), this.m_reverseHistory.push(x);
				}
			}
			return this.WALKTRACE(L, m, h, N, y, g, f, k, _, v, u, n, a, c, s, l, M, o);
		}
		PrettifyChanges(t) {
	...
	var mi = class v1 {
		...
		static computeLinks(t, n = ci()) {
			...
			for (let a = 1, l = t.getLineCount(); a <= l; a++) {
				...
				let c = 0, m = 0, h = 0, g = 1, f = !1, b = !1, p = !1, _ = !1;
				for (; c < u;) {
					let v = !1;
					...
					if (g === 13) {
						...
						switch (L) {
							...
							case 34:
							case 96:
								h === L ? y = 1 : h === 39 || h === 34 || h === 96 ? y = 0 : y = 1;
								break;
							case 42:
								y = h === 42 ? 1 : 0;
								break;
						...
						y === 1 && (s.push(v1._createLink(r, o, a, m, c)), v = !0);
					} else if (g === 12) {
						let y;
						L === 91 ? (b = !0, y = 0) : y = r.get(L), y === 1 ? v = !0 : g = 13;
					} else g = n.nextState(g, L), g === 0 && (v = !0);
					v && (g = 1, f = !1, b = !1, _ = !1, m = c + 1, h = L), c++;
				}
	...
	const K = {
		...
		relative(e, t) {
			...
			if (n.length !== e.length || r.length !== t.length) {
				...
				const _ = b.length, v = p.length, L = _ < v ? _ : v;
				let y;
				for (y = 0; y < L && b[y].toLowerCase() === p[y].toLowerCase(); y++);
				return y === 0 ? r : y === L ? v > L ? p.slice(y).join("\\") : _ > L ? "..\\".repeat(_ - 1 - y) + ".." : "" : "..\\".repeat(_ - y) + p.slice(y).join("\\");
			}
			let s = 0;
	...
	var aa = class {
		static computeUnicodeHighlights(e, t, n) {
			...
			e: for (let p = r, _ = s; p <= _; p++) {
				const v = e.getLineContent(p), L = v.length;
				u.reset(0);
				do
					if (h = u.next(v), h) {
						let y = h.index, N = h.index + h[0].length;
						y > 0 && ct(v.charCodeAt(y - 1)) && y--, N + 1 < L && ct(v.charCodeAt(N - 1)) && N++;
						const k = v.substring(y, N);
						let M = l1(y + 1, dr, v, 0);
						M && M.endColumn <= y + 1 && (M = null);
						const w = a.shouldHighlightNonBasicASCII(k, M ? M.word : null);
						if (w !== 0) {
							if (w === 3 ? g++ : w === 2 ? f++ : w === 1 ? b++ : us(), c.length >= 1e3) {
								m = !0;
								break e;
							}
							c.push(new I(p, y + 1, p, N + 1));
						}
					}
				while (h);
			}
...
`, `
`), s = $s(n, r), a = c1.ofText(n.substring(0, s)).addToPosition(this.range.getStartPosition()), l = r.substring(s), o = I.fromPositions(a, this.range.getEndPosition());
			return new xe(o, l);
		}
...
`), r = this.text.replaceAll(`\r
`, `
`), s = zs(n, r), a = c1.ofText(n.substring(0, n.length - s)).addToPosition(this.range.getStartPosition()), l = r.substring(0, r.length - s), o = I.fromPositions(this.range.getStartPosition(), a);
			return new xe(o, l);
		}
		toString() {
	...
	var Qe = class Et extends Ie {
		static fromRangeMappings(t) {
			const n = q.join(t.map((s) => q.fromRangeInclusive(s.originalRange))), r = q.join(t.map((s) => q.fromRangeInclusive(s.modifiedRange)));
			return new Et(n, r, t);
		}
		constructor(t, n, r) {
	...
	function va(e, t, n) {
		...
		return new Qe(new q(e.originalRange.startLineNumber + r, e.originalRange.endLineNumber + 1 + s), new q(e.modifiedRange.startLineNumber + r, e.modifiedRange.endLineNumber + 1 + s), [e]);
	}
	const La = 3;
	var Na = class {
		computeDiff(e, t, n) {
		...
		static createFromDiffResult(t, n, r, s, a, l, o) {
			...
			if (n.originalLength === 0 ? (u = r.getStartLineNumber(n.originalStart) - 1, c = 0) : (u = r.getStartLineNumber(n.originalStart), c = r.getEndLineNumber(n.originalStart + n.originalLength - 1)), n.modifiedLength === 0 ? (m = s.getStartLineNumber(n.modifiedStart) - 1, h = 0) : (m = s.getStartLineNumber(n.modifiedStart), h = s.getEndLineNumber(n.modifiedStart + n.modifiedLength - 1)), l && n.originalLength > 0 && n.originalLength < 20 && n.modifiedLength > 0 && n.modifiedLength < 20 && a()) {
				...
				if (f.getElements().length > 0 && b.getElements().length > 0) {
					...
					o && (p = Ra(p)), g = [];
					for (let _ = 0, v = p.length; _ < v; _++) g.push(vt.createFromDiffChange(p[_], f, b));
				}
			}
			return new ns(u, c, m, h, g);
		}
	}, Aa = class {
		constructor(e, t, n) {
				...
				for (; s < c && a < m;) {
					...
					if (h !== g) {
						...
						}
						{
							...
							const p = h.length + 1, _ = g.length + 1;
							for (; f < p && b < _ && h.charCodeAt(f - 1) === h.charCodeAt(b - 1);) f++, b++;
							(f < p || b < _) && this._pushTrimWhitespaceCharChange(r, s + 1, f, p, a + 1, b, _);
						}
					}
					s++, a++;
				}
...
`);
		...
		compute(e, t, n = g1.instance, r) {
			...
			for (let f = 0; f < e.length; f++) for (let b = 0; b < t.length; b++) {
				...
				const p = f === 0 ? 0 : s.get(f - 1, b), _ = b === 0 ? 0 : s.get(f, b - 1);
				let v;
				e.getElement(f) === t.getElement(b) ? (f === 0 || b === 0 ? v = 0 : v = s.get(f - 1, b - 1), f > 0 && b > 0 && a.get(f - 1, b - 1) === 3 && (v += l.get(f - 1, b - 1)), v += r ? r(f, b) : 1) : v = -1;
				const L = Math.max(p, _, v);
				if (L === v) {
					const y = f > 0 && b > 0 ? l.get(f - 1, b - 1) : 0;
					l.set(f, b, y + 1), a.set(f, b, 3);
				} else L === p ? (l.set(f, b, 0), a.set(f, b, 1)) : L === _ && (l.set(f, b, 0), a.set(f, b, 2));
				s.set(f, b, L);
			}
			const o = [];
		...
		compute(e, t, n = g1.instance) {
			...
			e: for (;;) {
				...
				for (c = b; c <= p; c += 2) {
					const _ = c === p ? -1 : o.get(c + 1), v = c === b ? -1 : o.get(c - 1) + 1, L = Math.min(Math.max(_, v), r.length), y = L - c;
					if (L > r.length || y > s.length) continue;
					const N = a(L, y);
					o.set(c, N);
					const k = L === _ ? u.get(c + 1) : u.get(c - 1);
					if (u.set(c, N !== L ? new xr(k, L, y, N - L) : k), o.get(c) === r.length && o.get(c) - c === s.length) break e;
				}
			}
			let m = u.get(c);
			const h = [];
...
`).length >= 15 && Da(m, (h) => h.length >= 2) >= 2;
	...
	function Fa(e, t, n, r, s, a) {
		...
		for (const g of e) {
			...
			for (let b = g.modified.startLineNumber; b < g.modified.endLineNumberExclusive - 2; b++) {
				const p = `${n[b - 1]}:${n[b + 1 - 1]}:${n[b + 2 - 1]}`, _ = new q(b, b + 3), v = [];
				o.forEach(p, ({ range: L }) => {
					for (const N of f) if (N.originalLineRange.endLineNumberExclusive + 1 === L.endLineNumberExclusive && N.modifiedLineRange.endLineNumberExclusive + 1 === _.endLineNumberExclusive) {
						N.originalLineRange = new q(N.originalLineRange.startLineNumber, L.endLineNumberExclusive), N.modifiedLineRange = new q(N.modifiedLineRange.startLineNumber, _.endLineNumberExclusive), v.push(N);
						return;
					}
					const y = {
						modifiedLineRange: _,
		...
		u.sort(ga(je((g) => g.modifiedLineRange.length, Xe)));
		const c = new wt(), m = new wt();
		for (const g of u) {
			const f = g.modifiedLineRange.startLineNumber - g.originalLineRange.startLineNumber, b = c.subtractFrom(g.modifiedLineRange), p = m.subtractFrom(g.originalLineRange).getWithDelta(f), _ = b.getIntersection(p);
			for (const v of _.ranges) {
				if (v.length < 3) continue;
				const L = v, y = v.delta(-f);
				l.push(new Ie(y, L)), c.addRange(L), m.addRange(y);
			}
		}
		l.sort(je((g) => g.original.startLineNumber, Xe));
		const h = new _r(e);
		for (let g = 0; g < l.length; g++) {
			const f = l[g], b = h.findLastMonotonous((M) => M.original.startLineNumber <= f.original.startLineNumber), p = Te(e, (M) => M.modified.startLineNumber <= f.modified.startLineNumber), _ = Math.max(f.original.startLineNumber - b.original.startLineNumber, f.modified.startLineNumber - p.modified.startLineNumber), v = h.findLastMonotonous((M) => M.original.startLineNumber < f.original.endLineNumberExclusive), L = Te(e, (M) => M.modified.startLineNumber < f.modified.endLineNumberExclusive), y = Math.max(v.original.endLineNumberExclusive - f.original.endLineNumberExclusive, L.modified.endLineNumberExclusive - f.modified.endLineNumberExclusive);
			let N;
			for (N = 0; N < _; N++) {
			...
			k > 0 && (m.addRange(new q(f.original.endLineNumberExclusive, f.original.endLineNumberExclusive + k)), c.addRange(new q(f.modified.endLineNumberExclusive, f.modified.endLineNumberExclusive + k))), (N > 0 || k > 0) && (l[g] = new Ie(new q(f.original.startLineNumber - N, f.original.endLineNumberExclusive + k), new q(f.modified.startLineNumber - N, f.modified.endLineNumberExclusive + k)));
		}
		return l;
	}
	function Pr(e, t, n) {
	...
	function Ir(e, t, n, r, s = !1) {
		...
		function u(c, m) {
			...
			let p = b.seq1Range.length, _ = b.seq2Range.length;
			for (; a.length > 0;) {
				const v = a[0];
				if (!(v.seq1Range.intersects(f.seq1Range) || v.seq2Range.intersects(f.seq2Range))) break;
				const L = new re(r(e, v.seq1Range.start), r(t, v.seq2Range.start)), y = L.intersect(v);
				if (p += y.seq1Range.length, _ += y.seq2Range.length, f = f.join(L), f.seq1Range.endExclusive >= v.seq1Range.endExclusive) a.shift();
				else break;
			}
			(s && p + _ < f.seq1Range.length + f.seq2Range.length || p + _ < (f.seq1Range.length + f.seq2Range.length) * 2 / 3) && l.push(f), o = f.getEndExclusives();
		}
		for (; a.length > 0;) {
			const c = a.shift();
	...
	function Wa(e, t, n) {
		...
		do {
			...
			for (let c = 1; c < r.length; c++) {
				let g = function(f, b) {
					...
					const v = e.countLinesIn(f.seq1Range), L = f.seq1Range.length, y = t.countLinesIn(f.seq2Range), N = f.seq2Range.length, k = e.countLinesIn(b.seq1Range), M = b.seq1Range.length, w = t.countLinesIn(b.seq2Range), C = b.seq2Range.length, T = 130;
					function U(x) {
						return Math.min(x, T);
					}
					return Math.pow(Math.pow(U(v * 40 + L), 1.5) + Math.pow(U(y * 40 + N), 1.5), 1.5) + Math.pow(Math.pow(U(k * 40 + M), 1.5) + Math.pow(U(w * 40 + C), 1.5), 1.5) > (T ** 1.5) ** 1.5 * 1.3;
				};
		...
		return ma(r, (u, c, m) => {
			...
			const _ = re.fromOffsetPairs(u ? u.getEndExclusives() : Se.zero, m ? m.getStarts() : Se.max), v = h.intersect(_);
...
`);
	...
	var $a = class {
		...
		computeDiff(e, t, n) {
			...
			let _ = 0, v = 0;
			for (const N of g) {
				st(() => N.seq1Range.start - _ === N.seq2Range.start - v), p(N.seq1Range.start - _), _ = N.seq1Range.endExclusive, v = N.seq2Range.endExclusive;
			...
			let y = [];
			return n.computeMoves && (y = this.computeMoves(L, e, t, o, u, r, s, n)), st(() => {
				function N(M, w) {
		...
		refineDiff(e, t, n, r, s, a) {
			...
			let m = c.diffs;
			return m = Dr(o, u, m), m = Ir(o, u, m, (h, g) => h.findWordContaining(g)), a.extendToSubwords && (m = Ir(o, u, m, (h, g) => h.findSubWordContaining(g), !0)), m = qa(o, u, m), m = Wa(o, u, m), {
	...
	(function(e) {
		(function(t) {
			(function(n) {
				...
				n.formatHex = u;
				function c(p, _ = !1) {
					return _ && p.rgba.a === 1 ? e.Format.CSS.formatHex(p) : `#${o(p.rgba.r)}${o(p.rgba.g)}${o(p.rgba.b)}${o(Math.round(p.rgba.a * 255))}`;
				}
				n.formatHexA = c;
				function m(p) {
				...
				function h(p) {
					...
					if (p.startsWith("rgba(")) {
						...
						if (!_) throw new Error("Invalid color format " + p);
						return new e(new d(parseInt(_.groups?.r ?? "0"), parseInt(_.groups?.g ?? "0"), parseInt(_.groups?.b ?? "0"), parseFloat(_.groups?.a ?? "0")));
					}
					if (p.startsWith("rgb(")) {
						const _ = p.match(/rgb\((?<r>(?:\+|-)?\d+), *(?<g>(?:\+|-)?\d+), *(?<b>(?:\+|-)?\d+)\)/);
						if (!_) throw new Error("Invalid color format " + p);
						return new e(new d(parseInt(_.groups?.r ?? "0"), parseInt(_.groups?.g ?? "0"), parseInt(_.groups?.b ?? "0")));
					}
					return g(p);
				}
				n.parse = h;
				...
				function f(p) {
					const _ = p.length;
					if (_ === 0 || p.charCodeAt(0) !== 35) return null;
					if (_ === 7) return new e(new d(16 * b(p.charCodeAt(1)) + b(p.charCodeAt(2)), 16 * b(p.charCodeAt(3)) + b(p.charCodeAt(4)), 16 * b(p.charCodeAt(5)) + b(p.charCodeAt(6)), 1));
					if (_ === 9) return new e(new d(16 * b(p.charCodeAt(1)) + b(p.charCodeAt(2)), 16 * b(p.charCodeAt(3)) + b(p.charCodeAt(4)), 16 * b(p.charCodeAt(5)) + b(p.charCodeAt(6)), (16 * b(p.charCodeAt(7)) + b(p.charCodeAt(8))) / 255));
					if (_ === 4) {
						const v = b(p.charCodeAt(1)), L = b(p.charCodeAt(2)), y = b(p.charCodeAt(3));
						return new e(new d(16 * v + v, 16 * L + L, 16 * y + y));
					}
					if (_ === 5) {
						const v = b(p.charCodeAt(1)), L = b(p.charCodeAt(2)), y = b(p.charCodeAt(3)), N = b(p.charCodeAt(4));
						return new e(new d(16 * v + v, 16 * L + L, 16 * y + y, (16 * N + N) / 255));
					}
					return null;
				}
				n.parseHex = f;
...
`), p = b.length, _ = f + p - 1, v = h.lastIndexOf(`
`) + 1, L = m.index - v + 1, y = b[b.length - 1], N = {
					range: {
						startLineNumber: f,
						startColumn: L,
						endLineNumber: _,
						endColumn: p === 1 ? L + m[0].length : y.length + 1
					},
					text: (m.groups ?? {}).label ?? "",
	...
	var r0 = class {
		...
		async $computeMoreMinimalEdits(t, n, r) {
			...
			for (let { range: u, text: c, eol: m } of n) {
				...
				const g = ai(h, c, r), f = s.offsetAt(I.lift(u).getStartPosition());
				for (const b of g) {
					const p = s.positionAt(f + b.originalStart), _ = s.positionAt(f + b.originalStart + b.originalLength), v = {
...
//# sourceMappingURL=editor.worker-D7aP5u3t.js.map