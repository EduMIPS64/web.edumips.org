...
` + e.stack) : e;
	...
	var A1 = class extends Error {
		constructor() {
			super(as), this.name = this.message;
		}
	...
	}, Z = class zr extends Error {
		constructor(t) {
			super(t || "An unexpected bug occurred."), Object.setPrototypeOf(this, zr.prototype);
		}
	};
	function os(e, t = "Unreachable") {
		throw new Error(t);
	}
	function us(e, t = "unexpected state") {
	...
	(function(e) {
		...
		function m(w, L) {
			let x = 0;
			for (const q of w) if (L(q, x++)) return !0;
			return !1;
		}
		e.some = m;
		function h(w, L) {
			let x = 0;
			for (const q of w) if (!L(q, x++)) return !1;
			return !0;
		}
		e.every = h;
		function g(w, L) {
		...
		e.flatMap = p;
		function* y(...w) {
			for (const L of w) hs(L) ? yield* L : yield L;
		}
		e.concat = y;
		function v(w, L, x) {
			let q = x;
			for (const C of w) q = L(q, C);
			return q;
		}
		e.reduce = v;
		function N(w) {
		...
		e.length = N;
		function* _(w, L, x = w.length) {
			for (L < -w.length && (L = 0), L < 0 && (L += w.length), x < 0 ? x += w.length : x > w.length && (x = w.length); L < x; L++) yield w[L];
		}
		e.slice = _;
		function M(w, L = Number.POSITIVE_INFINITY) {
			const x = [];
			if (L === 0) return [x, w];
			const q = w[Symbol.iterator]();
			for (let C = 0; C < L; C++) {
				const R = q.next();
				if (R.done) return [x, e.empty()];
				x.push(R.value);
			}
	...
	var It = class Or {
		...
		add(t) {
			...
			return this._isDisposed ? Or.DISABLE_DISPOSED_WARNING || console.warn((/* @__PURE__ */ new Error("Trying to add a disposable to a DisposableStore that has already been disposed of. The added object will be leaked!")).stack) : this._toDispose.add(t), t;
		}
		delete(t) {
			if (t) {
	...
	const ds = globalThis.performance.now.bind(globalThis.performance);
	var k1 = class Gr {
		static create(t) {
			return new Gr(t);
		}
		constructor(t) {
			this._now = t === !1 ? Date.now : ds, this._startTime = this._now(), this._stopTime = -1;
		}
		stop() {
	...
	(function(e) {
		e.None = () => $e.None;
		function t(R, S) {
			return g(R, () => {}, 0, void 0, !0, void 0, S);
		}
		e.defer = t;
		function n(R) {
			return (S, E = null, A) => {
				let T = !1, B;
				return B = R((H) => {
					if (!T) return B ? B.dispose() : T = !0, S.call(E, H);
				}, null, A), T && B.dispose(), B;
			};
		}
		e.once = n;
		function r(R, S) {
		...
		e.onceIf = r;
		function s(R, S, E) {
			return m((A, T = null, B) => R((H) => A.call(T, S(H)), null, B), E);
		}
		e.map = s;
		function a(R, S, E) {
			return m((A, T = null, B) => R((H) => {
				S(H), A.call(T, H);
			}, null, B), E);
		}
		e.forEach = a;
		function l(R, S, E) {
			return m((A, T = null, B) => R((H) => S(H) && A.call(T, H), null, B), E);
		}
		e.filter = l;
		function o(R) {
			return R;
		...
		e.signal = o;
		function u(...R) {
			return (S, E = null, A) => h(ms(...R.map((T) => T((B) => S.call(E, B)))), A);
		}
		e.any = u;
		function c(R, S, E, A) {
			let T = E;
			return s(R, (B) => (T = S(T, B), T), A);
		}
		e.reduce = c;
		function m(R, S) {
			let E;
		...
		function g(R, S, E = 100, A = !1, T = !1, B, H) {
			let Y, J, Ae, Rt = 0, Ze;
			const At = new ue({
				leakWarningThreshold: B,
				onWillAddFirstListener() {
					Y = R((m0) => {
						Rt++, J = S(J, m0), A && !Ae && (At.fire(J), J = void 0), Ze = () => {
							const f0 = J;
							J = void 0, Ae = void 0, (!A || Rt > 1) && At.fire(f0), Rt = 0;
		...
		e.debounce = g;
		function f(R, S = 0, E) {
			return e.debounce(R, (A, T) => A ? (A.push(T), A) : [T], S, void 0, !0, void 0, E);
		}
		e.accumulate = f;
		function b(R, S = (A, T) => A === T, E) {
			let A = !0, T;
			return l(R, (B) => {
				const H = A || !S(B, T);
				return A = !1, T = B, H;
		...
		class _ {
			...
			filter(S) {
				return this.steps.push((E) => S(E) ? E : N), this;
			}
			reduce(S, E) {
				let A = E;
				return this.steps.push((T) => (A = S(A, T), A)), this;
			}
			latch(S = (E, A) => E === A) {
				let E = !0, A;
				return this.steps.push((T) => {
					const B = E || !S(T, A);
					return E = !1, A = T, B ? T : N;
				}), this;
			}
			evaluate(S) {
				for (const E of this.steps) if (S = E(S), S === N) break;
				return S;
			}
		}
		function M(R, S, E = (A) => A) {
			const A = (...Y) => H.fire(E(...Y)), T = () => R.on(S, A), B = () => R.removeListener(S, A), H = new ue({
		...
		function F(R, S, E = (A) => A) {
			const A = (...Y) => H.fire(E(...Y)), T = () => R.addEventListener(S, A), B = () => R.removeEventListener(S, A), H = new ue({
				onWillAddFirstListener: T,
				onDidRemoveLastListener: B
			});
			return H.event;
		}
		e.fromDOMEventEmitter = F;
		function k(R, S) {
			let E;
			const A = new Promise((T, B) => {
				const H = n(R)(T, null, S);
				E = () => H.dispose();
			});
			return A.cancel = E, A;
		}
		...
		function q(R, S) {
			return new x(R, S).emitter.event;
		}
		e.fromObservable = q;
		function C(R) {
			return (S, E, A) => {
				...
				const H = {
					...
					endUpdate() {
						T--, T === 0 && (R.reportChanges(), B && (B = !1, S.call(E)));
					},
	...
	var ws = class jr {
		...
		static create() {
			return new Xr((/* @__PURE__ */ new Error()).stack ?? "");
		}
		constructor(t) {
			this.value = t;
		}
...
`));
	...
	var ue = class {
		constructor(e) {
			this._size = 0, this._options = e, this._leakageMon = this._options?.leakWarningThreshold ? new ws(e?.onListenerError ?? rt, this._options?.leakWarningThreshold ?? bs) : void 0, this._perfMon = this._options?._profName ? new ps(this._options._profName) : void 0, this._deliveryQueue = this._options?.deliveryQueue;
		}
		dispose() {
		...
		get event() {
			return this._event ??= (e, t, n) => {
				if (this._leakageMon && this._size > this._leakageMon.threshold ** 2) {
					...
					console.warn(l);
					const o = this._leakageMon.getMostFrequentStack() ?? ["UNKNOWN stack", -1], u = new vs(`${l}. HINT: Stack shows most frequent listener (${o[1]}-times)`, o[0]);
				...
				const r = new Bt(e);
				let s;
				this._leakageMon && this._size >= Math.ceil(this._leakageMon.threshold * .2) && (r.stack = ys.create(), s = this._leakageMon.check(r.stack, this._size + 1)), this._listeners ? this._listeners instanceof Bt ? (this._deliveryQueue ??= new Ns(), this._listeners = [this._listeners, r]) : this._listeners.push(r) : (this._options?.onWillAddFirstListener?.(this), this._listeners = r, this._options?.onDidAddFirstListener?.(this)), this._options?.onDidAddListener?.(this), this._size++;
				const a = at(() => {
	...
	const Rs = M1() === "pseudo" || typeof document < "u" && document.location && typeof document.location.hash == "string" && document.location.hash.indexOf("pseudo=true") >= 0;
	function E1(e, t) {
		let n;
	...
	function D(e, t, ...n) {
		return E1(typeof e == "number" ? As(e, t) : t, n);
	}
	function As(e, t) {
		const n = Ss()?.[e];
		if (typeof n != "string") {
	...
	const Ms = typeof ee?.versions?.electron == "string" && ee?.type === "renderer";
	if (typeof ee == "object") {
	...
	} else typeof navigator == "object" && !Ms ? (he = navigator.userAgent, lt = he.indexOf("Windows") >= 0, ot = he.indexOf("Macintosh") >= 0, (he.indexOf("Macintosh") >= 0 || he.indexOf("iPad") >= 0 || he.indexOf("iPhone") >= 0) && navigator.maxTouchPoints && navigator.maxTouchPoints, ut = he.indexOf("Linux") >= 0, he?.indexOf("Mobi"), P1 = !0, M1(), navigator.language.toLowerCase()) : console.error("Unable to resolve platform.");
	const ze = lt, Es = ot;
	P1 && typeof me.importScripts == "function" && me.origin;
	const ce = he, Ps = typeof me.postMessage == "function" && !me.importScripts;
	(() => {
		if (Ps) {
			const e = [];
	...
	const Ts = !!(ce && ce.indexOf("Chrome") >= 0);
	ce && ce.indexOf("Firefox");
	!Ts && ce && ce.indexOf("Safari");
	ce && ce.indexOf("Edg/");
	...
	var Is = class {
		constructor(e, t) {
			this.lastCache = void 0, this.lastArgKey = void 0, typeof e == "function" ? (this._fn = e, this._computeKey = Fs) : (this._fn = t, this._computeKey = e.getCacheKey);
		}
		get(e) {
	...
	var $t = class et {
		...
		static {
			this.cache = new Is({ getCacheKey: JSON.stringify }, (t) => {
				...
				function r(c, m) {
					const h = new Map(c);
					for (const [g, f] of m) h.set(g, f);
					return h;
				}
				function s(c, m) {
					if (!c) return m;
					const h = /* @__PURE__ */ new Map();
					for (const [g, f] of c) m.has(g) && h.set(g, f);
					return h;
				}
				const a = this.ambiguousCharacterData.value;
				let l = t.filter((c) => !c.startsWith("_") && Object.hasOwn(a, c));
		...
		static {
			this._locales = new Ht(() => Object.keys(et.ambiguousCharacterData.value).filter((t) => !t.startsWith("_")));
		}
		static getLocales() {
	...
	const Ot = "default", Xs = "$initialize";
	var Qs = class {
		...
		async sendMessage(e, t, n) {
			...
			return new Promise((s, a) => {
				this._pendingReplies[r] = {
					resolve: s,
					reject: a
				}, this._send(new Qs(this._workerId, r, e, t, n));
			});
		}
		listen(e, t, n) {
			let r = null;
			const s = new ue({
				onWillAddFirstListener: () => {
					r = String(++this._lastSentReq), this._pendingEmitters.set(r, s), this._send(new Ys(this._workerId, r, e, t, n));
				},
				onDidRemoveLastListener: () => {
					this._pendingEmitters.delete(r), this._send(new Zs(this._workerId, r)), r = null;
				}
			});
			return s.event;
		}
		...
		_handleSubscribeEventMessage(e) {
			const t = e.req, n = this._handler.handleEvent(e.channel, e.eventName, e.arg)((r) => {
				this._send(new Js(this._workerId, t, r));
			});
	...
	var ei = class {
		constructor(e, t) {
			this._localChannels = /* @__PURE__ */ new Map(), this._remoteChannels = /* @__PURE__ */ new Map(), this._protocol = new Ks({
				sendMessage: (n, r) => {
					e(n, r);
				},
		...
		_handleMessage(e, t, n) {
			if (e === Ot && t === Xs) return this.initialize(n[0]);
	...
	function ti(e) {
		...
		B1 = !0;
		const t = new ei((n) => globalThis.postMessage(n), (n) => e(n));
	...
	new Ht(() => /* @__PURE__ */ new Uint8Array(256));
	let Gt;
	var ni = class Qr {
		static wrap(t) {
			return q1 && !Buffer.isBuffer(t) && (t = Buffer.from(t.buffer, t.byteOffset, t.byteLength)), new Qr(t);
		}
		constructor(t) {
	...
	function Oe(e, t = 32) {
		return e instanceof ArrayBuffer ? ri(ni.wrap(new Uint8Array(e))) : (e >>> 0).toString(16).padStart(t / 4, "0");
	}
	(class Yr {
		static {
		...
		_step() {
			const t = Yr._bigBlock32, n = this._buffDV;
	...
	var ke = class {
		...
		static _getElements(t) {
			...
			if (Ue._isStringArray(n)) {
				...
				for (let s = 0, a = n.length; s < a; s++) r[s] = si(n[s], 0);
				return [
					n,
					r,
					!0
		...
		ComputeDiffRecursive(t, n, r, s, a) {
			...
			if (!a[0]) {
				...
				let g = [];
				return a[0] ? g = [new we(c + 1, n - (c + 1) + 1, m + 1, s - (m + 1) + 1)] : g = this.ComputeDiffRecursive(c + 1, n, m + 1, s, a), this.ConcatenateChanges(h, g);
			}
			return [new we(t, n - t + 1, r, s - r + 1)];
		}
		WALKTRACE(t, n, r, s, a, l, o, u, c, m, h, g, f, b, p, y, v, N) {
			let _ = null, M = null, F = new $1(), k = n, w = r, L = f[0] - y[0] - s, x = -1073741824, q = this.m_forwardHistory.length - 1;
			do {
				const C = L + t;
				C === k || C < w && c[C - 1] < c[C + 1] ? (h = c[C + 1], b = h - L - s, h < x && F.MarkNextChange(), x = h, F.AddModifiedElement(h + 1, b), L = C + 1 - t) : (h = c[C - 1] + 1, b = h - L - s, h < x && F.MarkNextChange(), x = h - 1, F.AddOriginalElement(h, b + 1), L = C - 1 - t), q >= 0 && (c = this.m_forwardHistory[q], t = c[0], k = 1, w = c.length - 1);
			} while (--q >= -1);
			if (_ = F.getReverseChanges(), N[0]) {
				let C = f[0] + 1, R = y[0] + 1;
				if (_ !== null && _.length > 0) {
					const S = _[_.length - 1];
					C = Math.max(C, S.getOriginalEnd()), R = Math.max(R, S.getModifiedEnd());
				}
				M = [new we(C, g - C + 1, R, p - R + 1)];
			} else {
				F = new $1(), k = l, w = o, L = f[0] - y[0] - u, x = 1073741824, q = v ? this.m_reverseHistory.length - 1 : this.m_reverseHistory.length - 2;
				do {
					const C = L + a;
					C === k || C < w && m[C - 1] >= m[C + 1] ? (h = m[C + 1] - 1, b = h - L - u, h > x && F.MarkNextChange(), x = h + 1, F.AddOriginalElement(h + 1, b + 1), L = C + 1 - a) : (h = m[C - 1], b = h - L - u, h > x && F.MarkNextChange(), x = h, F.AddModifiedElement(h + 1, b + 1), L = C - 1 - a), q >= 0 && (m = this.m_reverseHistory[q], a = m[0], k = 1, w = m.length - 1);
				} while (--q >= -1);
				M = F.getChanges();
			}
			return this.ConcatenateChanges(_, M);
		}
		ComputeRecursionPoint(t, n, r, s, a, l, o) {
			let u = 0, c = 0, m = 0, h = 0, g = 0, f = 0;
			t--, r--, a[0] = 0, l[0] = 0, this.m_forwardHistory = [], this.m_reverseHistory = [];
			const b = n - t + (s - r), p = b + 1, y = new Int32Array(p), v = new Int32Array(p), N = s - r, _ = n - t, M = t - r, F = n - s, k = (_ - N) % 2 === 0;
			y[N] = t, v[_] = n, o[0] = !1;
			for (let w = 1; w <= b / 2 + 1; w++) {
				let L = 0, x = 0;
				m = this.ClipDiagonalBound(N - w, w, N, p), h = this.ClipDiagonalBound(N + w, w, N, p);
				for (let C = m; C <= h; C += 2) {
					C === m || C < h && y[C - 1] < y[C + 1] ? u = y[C + 1] : u = y[C - 1] + 1, c = u - (C - N) - M;
					const R = u;
					for (; u < n && c < s && this.ElementsAreEqual(u + 1, c + 1);) u++, c++;
					if (y[C] = u, u + c > L + x && (L = u, x = c), !k && Math.abs(C - _) <= w - 1 && u >= v[C]) return a[0] = u, l[0] = c, R <= v[C] && w <= 1448 ? this.WALKTRACE(N, m, h, M, _, g, f, F, y, v, u, n, a, c, s, l, k, o) : null;
				}
				const q = (L - t + (x - r) - w) / 2;
				if (this.ContinueProcessingPredicate !== null && !this.ContinueProcessingPredicate(L, q)) return o[0] = !0, a[0] = L, l[0] = x, q > 0 && w <= 1448 ? this.WALKTRACE(N, m, h, M, _, g, f, F, y, v, u, n, a, c, s, l, k, o) : (t++, r++, [new we(t, n - t + 1, r, s - r + 1)]);
				g = this.ClipDiagonalBound(_ - w, w, _, p), f = this.ClipDiagonalBound(_ + w, w, _, p);
				for (let C = g; C <= f; C += 2) {
					C === g || C < f && v[C - 1] >= v[C + 1] ? u = v[C + 1] - 1 : u = v[C - 1], c = u - (C - _) - F;
					const R = u;
					for (; u > t && c > r && this.ElementsAreEqual(u, c);) u--, c--;
					if (v[C] = u, k && Math.abs(C - N) <= w && u <= y[C]) return a[0] = u, l[0] = c, R >= y[C] && w <= 1448 ? this.WALKTRACE(N, m, h, M, _, g, f, F, y, v, u, n, a, c, s, l, k, o) : null;
				}
				if (w <= 1447) {
					let C = new Int32Array(h - m + 2);
					C[0] = N - m + 1, Me.Copy2(y, m, C, 1, h - m + 1), this.m_forwardHistory.push(C), C = new Int32Array(f - g + 2), C[0] = _ - g + 1, Me.Copy2(v, g, C, 1, f - g + 1), this.m_reverseHistory.push(C);
				}
			}
			return this.WALKTRACE(N, m, h, M, _, g, f, F, y, v, u, n, a, c, s, l, k, o);
		}
		PrettifyChanges(t) {
			...
			for (let n = t.length - 1; n >= 0; n--) {
				...
				for (let h = 1;; h++) {
					const g = r.originalStart - h, f = r.modifiedStart - h;
					if (g < s || f < a || l && !this.OriginalElementsAreEqual(g, g + r.originalLength) || o && !this.ModifiedElementsAreEqual(f, f + r.modifiedLength)) break;
					const b = (g === s && f === a ? 5 : 0) + this._boundaryScore(g, r.originalLength, f, r.modifiedLength);
					b > c && (c = b, u = h);
				}
			...
			if (this._hasStrings) for (let n = 1, r = t.length; n < r; n++) {
				const s = t[n - 1], a = t[n], l = a.originalStart - s.originalStart - s.originalLength, o = s.originalStart, u = a.originalStart + a.originalLength, c = u - o, m = s.modifiedStart, h = a.modifiedStart + a.modifiedLength, g = h - m;
				if (l < 5 && c < 20 && g < 20) {
					const f = this._findBetterContiguousSequence(o, c, m, g, l);
					if (f) {
						const [b, p] = f;
		...
		_findBetterContiguousSequence(t, n, r, s, a) {
			...
			let u = 0, c = 0, m = 0;
			for (let h = t; h < l; h++) for (let g = r; g < o; g++) {
				const f = this._contiguousSequenceScore(h, g, a);
				f > 0 && f > u && (u = f, c = h, m = g);
			}
			return u > 0 ? [c, m] : null;
		}
	...
	var ai = class Jr {
		constructor(t) {
			const n = O1(t);
			this._defaultValue = n, this._asciiMap = Jr._createAsciiMap(n), this._map = /* @__PURE__ */ new Map();
		}
				...
				for (; c < u;) {
					...
					if (g === 13) {
						...
						switch (N) {
							...
							case 34:
							case 96:
								h === N ? _ = 1 : h === 39 || h === 34 || h === 96 ? _ = 0 : _ = 1;
								break;
							case 42:
								_ = h === 42 ? 1 : 0;
								break;
						...
						let _;
						N === 91 ? (b = !0, _ = 0) : _ = r.get(N), _ === 1 ? v = !0 : g = 13;
					} else g = n.nextState(g, N), g === 0 && (v = !0);
					v && (g = 1, f = !1, b = !1, y = !1, m = c + 1, h = N), c++;
				}
				g === 13 && s.push(v1._createLink(r, o, a, m, u));
			}
			return s;
		}
	};
	function mi(e) {
		return !e || typeof e.getLineCount != "function" || typeof e.getLineContent != "function" ? [] : hi.computeLinks(e);
	}
	var fi = class Zr {
		constructor() {
	...
	const ft = new Qt(), Yt = new Qt(), Jt = new Qt(), di = new Array(230), pi = Object.create(null), bi = Object.create(null), j1 = [];
	...
	(function() {
		...
		for (const s of t) {
			const [a, l, o, u, c, m, h, g, f] = s;
			if (r[l] || (r[l] = !0, pi[o] = l, bi[o.toLowerCase()] = l, a && (j1[l] = u)), !n[u]) {
				if (n[u] = !0, !c) throw new Error(`String representation missing for key code ${u} around scan code ${o}`);
				ft.define(u, c), Yt.define(u, g || c), Jt.define(u, f || g || c);
			}
			m && (di[m] = u);
		}
	})();
	...
	if (typeof Zt < "u" && typeof Zt.process < "u") {
		...
		get platform() {
			return ze ? "win32" : Es ? "darwin" : "linux";
		},
		get arch() {},
		get env() {
			return {};
	...
	const gt = Pe.cwd, yi = Pe.env, _i = Pe.platform, vi = 65, Li = 97, Ni = 90, Si = 122, Ne = 46, Q = 47, te = 92, fe = 58, Ri = 63;
	...
	function Y1(e, t) {
		Ai(t, "pathObject");
		const n = t.dir || t.root, r = t.base || `${t.name || ""}${Ci(t.ext)}`;
	...
	const K = {
		resolve(...e) {
			...
			for (let s = e.length - 1; s >= -1; s--) {
				...
				} else t.length === 0 ? a = gt() : (a = yi[`=${t}`] || gt(), (a === void 0 || a.slice(0, 2).toLowerCase() !== t.toLowerCase() && a.charCodeAt(2) === te) && (a = `${t}\\`));
				const l = a.length;
				let o = 0, u = "", c = !1;
				...
				if (l === 1) V(m) && (o = 1, c = !0);
					...
					if (h < l && h !== g) {
						const f = a.slice(g, h);
						for (g = h; h < l && V(a.charCodeAt(h));) h++;
						if (h < l && h !== g) {
							for (g = h; h < l && !V(a.charCodeAt(h));) h++;
							(h === l || h !== g) && (u = `\\\\${f}\\${a.slice(g, h)}`, o = h);
						}
					}
				} else o = 1;
		...
		relative(e, t) {
			...
			if (n.length !== e.length || r.length !== t.length) {
				...
				const y = b.length, v = p.length, N = y < v ? y : v;
				let _;
				for (_ = 0; _ < N && b[_].toLowerCase() === p[_].toLowerCase(); _++);
				return _ === 0 ? r : _ === N ? v > N ? p.slice(_).join("\\") : y > N ? "..\\".repeat(y - 1 - _) + ".." : "" : "..\\".repeat(y - _) + p.slice(_).join("\\");
			}
			let s = 0;
			...
			let h = -1, g = 0;
			for (; g < m; g++) {
				const b = e.charCodeAt(s + g);
				if (b !== t.charCodeAt(o + g)) break;
				b === te && (h = g);
			}
			if (g !== m) {
				if (h === -1) return r;
				...
				if (c > m) {
					if (t.charCodeAt(o + g) === te) return r.slice(o + g + 1);
					if (g === 2) return r.slice(o + g);
				}
				l > m && (e.charCodeAt(s + g) === te ? h = g : g === 2 && (h = 3)), h === -1 && (h = 0);
			}
			let f = "";
			for (g = s + h + 1; g <= a; ++g) (g === a || e.charCodeAt(g) === te) && (f += f.length === 0 ? ".." : "\\..");
		...
		toNamespacedPath(e) {
			...
			if (t.charCodeAt(0) === te) {
				if (t.charCodeAt(1) === te) {
					...
					if (n !== Ri && n !== Ne) return `\\\\?\\UNC\\${t.slice(2)}`;
				}
		...
		parse(e) {
			...
			if (V(s)) {
				if (r = 1, V(e.charCodeAt(1))) {
					...
					if (h < n && h !== g) {
						for (g = h; h < n && V(e.charCodeAt(h));) h++;
						if (h < n && h !== g) {
							for (g = h; h < n && !V(e.charCodeAt(h));) h++;
							h === n ? r = h : h !== g && (r = h + 1);
						}
	...
	function Pi(e, t) {
		...
		if (e.scheme && !ki.test(e.scheme)) throw new Error("[UriError]: Scheme contains illegal characters.");
		if (e.path) {
			if (e.authority) {
				if (!Mi.test(e.path)) throw new Error("[UriError]: If a URI contains an authority component, then the path component must either be empty or begin with a slash (\"/\") character");
			} else if (Ei.test(e.path)) throw new Error("[UriError]: If a URI does not contain an authority component, then the path cannot begin with two slash characters (\"//\")");
		}
	}
	function Di(e, t) {
		return !e && !t ? "file" : e;
	...
	function Ti(e, t) {
		switch (e) {
			case "https":
			case "http":
			case "file": t ? t[0] !== le && (t = le + t) : t = le;
		}
		return t;
	}
	const z = "", le = "/", Fi = /^(([^:/?#]+?):)?(\/\/([^/?#]*))?([^?#]*)(\?([^#]*))?(#(.*))?/;
	var e1 = class kt {
		...
		constructor(t, n, r, s, a, l = !1) {
			typeof t == "object" ? (this.scheme = t.scheme || z, this.authority = t.authority || z, this.path = t.path || z, this.query = t.query || z, this.fragment = t.fragment || z) : (this.scheme = Di(t, l), this.authority = n || z, this.path = Ti(this.scheme, r || z), this.query = s || z, this.fragment = a || z, Pi(this, l));
		}
		get fsPath() {
			return t1(this, !1);
		}
	...
	function ea(e, t, n, r, s) {
		return Zi(e, t, n, r, s) && Ki(e, t, n, r, s);
	}
	var ta = class {
		constructor(e, t) {
		...
		next(e) {
			...
			do {
				...
				if (r === this._prevMatchStartIndex && s === this._prevMatchLength) {
					if (s === 0) {
						zs(e, t, this._searchRegex.lastIndex) > 65535 ? this._searchRegex.lastIndex += 2 : this._searchRegex.lastIndex += 1;
						continue;
					}
					return null;
				}
				if (this._prevMatchStartIndex = r, this._prevMatchLength = s, !this._wordSeparators || ea(this._wordSeparators, e, t, r, s)) return n;
			} while (n);
			return null;
		}
	};
	...
	function ra(e = "") {
		let t = "(-?\\d*\\.\\d\\w*)|([^";
		for (const n of na) e.indexOf(n) >= 0 || (t += "\\" + n);
	...
	var ia = class {
		static computeUnicodeHighlights(e, t, n) {
			...
			let o;
			l === "allNonBasicAscii" ? o = /* @__PURE__ */ new RegExp("[^\\t\\n\\r\\x20-\\x7E]", "g") : o = new RegExp(`${aa(Array.from(l))}`, "g");
			const u = new ta(null, o), c = [];
			let m = !1, h, g = 0, f = 0, b = 0;
			e: for (let p = r, y = s; p <= y; p++) {
				const v = e.getLineContent(p), N = v.length;
				u.reset(0);
				do
					if (h = u.next(v), h) {
						let _ = h.index, M = h.index + h[0].length;
						if (_ > 0) ct(v.charCodeAt(_ - 1)) && _--;
						if (M + 1 < N) ct(v.charCodeAt(M - 1)) && M++;
						const F = v.substring(_, M);
						let k = l1(_ + 1, dr, v, 0);
						k && k.endColumn <= _ + 1 && (k = null);
						const w = a.shouldHighlightNonBasicASCII(F, k ? k.word : null);
						if (w !== 0) {
							if (w === 3 ? g++ : w === 2 ? f++ : w === 1 ? b++ : os(), c.length >= 1e3) {
								m = !0;
								break e;
							}
							c.push(new I(p, _ + 1, p, M + 1));
						}
					}
				while (h);
			}
		...
		static computeUnicodeHighlightReason(e, t) {
			...
			switch (n.shouldHighlightNonBasicASCII(e, null)) {
				...
				case 3: {
					const r = e.codePointAt(0), s = n.ambiguousCharacters.getPrimaryConfusable(r), a = $t.getLocales().filter((l) => !$t.getInstance(/* @__PURE__ */ new Set([...t.allowedLocales, l])).isAmbiguous(r));
					return {
						kind: 0,
	...
	function aa(e, t) {
		return `[${Vs(e.map((n) => String.fromCodePoint(n)).join(""))}]`;
	}
	var wr = class {
		constructor(e) {
		...
		shouldHighlightNonBasicASCII(e, t) {
			...
			if (t) for (const a of t) {
				const l = a.codePointAt(0), o = Gs(a);
...
` || e === "	";
	...
	var _r = class Kr {
		...
		static {
			this.compareByStart = je((t) => t.startLineNumber, Xe);
		}
		static joinMany(t) {
		...
		getUnion(t) {
			...
			for (; r < this._normalizedRanges.length || s < t._normalizedRanges.length;) {
				...
				a === null ? a = l : a.endLineNumberExclusive >= l.startLineNumber ? a = new U(a.startLineNumber, Math.max(a.endLineNumberExclusive, l.endLineNumberExclusive)) : (n.push(a), a = l);
			}
		...
		subtractFrom(t) {
			...
			for (let l = n; l < r; l++) {
				...
				o.startLineNumber > a && s.push(new U(a, o.startLineNumber)), a = o.endLineNumberExclusive;
			}
			return a < t.endLineNumberExclusive && s.push(new U(a, t.endLineNumberExclusive)), new He(s);
		}
		toString() {
...
` ? (n++, r = 0) : r++;
		...
		toLineRange() {
			return U.ofLength(1, this.lineCount + 1);
		}
		addToPosition(t) {
...
` && (this.lineStartOffsetByLineIdx.push(t + 1), t > 0 && e.charAt(t - 1) === "\r" ? this.lineEndOffsetByLineIdx.push(t - 1) : this.lineEndOffsetByLineIdx.push(t));
		...
		getPosition(e) {
			const t = Fe(this.lineStartOffsetByLineIdx, (s) => s <= e);
		...
		constructor(e, t) {
			us(t >= 1), super(), this._getLineContent = e, this._lineCount = t;
		}
		getValueOfRange(e) {
...
`, `
`), s = Ws(n, r), a = c1.ofText(n.substring(0, s)).addToPosition(this.range.getStartPosition()), l = r.substring(s), o = I.fromPositions(a, this.range.getEndPosition());
			return new xe(o, l);
		}
...
`), r = this.text.replaceAll(`\r
`, `
`), s = $s(n, r), a = c1.ofText(n.substring(0, n.length - s)).addToPosition(this.range.getStartPosition()), l = r.substring(0, r.length - s), o = I.fromPositions(this.range.getStartPosition(), a);
			return new xe(o, l);
		}
		toString() {
		...
		static inverse(t, n, r) {
			...
			let a = 1, l = 1;
			for (const u of t) {
				const c = new We(new U(a, u.original.startLineNumber), new U(l, u.modified.startLineNumber));
				c.modified.isEmpty || s.push(c), a = u.original.endLineNumberExclusive, l = u.modified.endLineNumberExclusive;
			}
			const o = new We(new U(a, n + 1), new U(l, r + 1));
	...
	var Qe = class Et extends Ie {
		static fromRangeMappings(t) {
			const n = U.join(t.map((s) => U.fromRangeInclusive(s.originalRange))), r = U.join(t.map((s) => U.fromRangeInclusive(s.modifiedRange)));
			return new Et(n, r, t);
		}
		constructor(t, n, r) {
		...
		toTextEdit(t) {
			...
			return new ya(this.originalRange, n);
		}
	};
	function Lr(e, t, n, r = !1) {
		const s = [];
		for (const a of ua(e.map((l) => _a(l, t, n)), (l, o) => l.original.intersectsOrTouches(o.original) || l.modified.intersectsOrTouches(o.modified))) {
	...
	function _a(e, t, n) {
		...
		return new Qe(new U(e.originalRange.startLineNumber + r, e.originalRange.endLineNumber + 1 + s), new U(e.modifiedRange.startLineNumber + r, e.modifiedRange.endLineNumber + 1 + s), [e]);
	}
	const va = 3;
	var La = class {
		computeDiff(e, t, n) {
			...
			for (const l of r.changes) {
				let o;
				l.originalEndLineNumber === 0 ? o = new U(l.originalStartLineNumber + 1, l.originalStartLineNumber + 1) : o = new U(l.originalStartLineNumber, l.originalEndLineNumber + 1);
				let u;
				l.modifiedEndLineNumber === 0 ? u = new U(l.modifiedStartLineNumber + 1, l.modifiedStartLineNumber + 1) : u = new U(l.modifiedStartLineNumber, l.modifiedEndLineNumber + 1);
	...
	function Sa(e) {
		...
		for (let r = 1, s = e.length; r < s; r++) {
			...
			Math.min(l, o) < va ? (n.originalLength = a.originalStart + a.originalLength - n.originalStart, n.modifiedLength = a.modifiedStart + a.modifiedLength - n.modifiedStart) : (t.push(a), n = a);
		}
		return t;
	}
	var h1 = class ts {
		...
		static createFromDiffResult(t, n, r, s, a, l, o) {
			...
			if (n.originalLength === 0 ? (u = r.getStartLineNumber(n.originalStart) - 1, c = 0) : (u = r.getStartLineNumber(n.originalStart), c = r.getEndLineNumber(n.originalStart + n.originalLength - 1)), n.modifiedLength === 0 ? (m = s.getStartLineNumber(n.modifiedStart) - 1, h = 0) : (m = s.getStartLineNumber(n.modifiedStart), h = s.getEndLineNumber(n.modifiedStart + n.modifiedLength - 1)), l && n.originalLength > 0 && n.originalLength < 20 && n.modifiedLength > 0 && n.modifiedLength < 20 && a()) {
				...
				if (f.getElements().length > 0 && b.getElements().length > 0) {
					...
					o && (p = Sa(p)), g = [];
					for (let y = 0, v = p.length; y < v; y++) g.push(vt.createFromDiffChange(p[y], f, b));
				}
			}
			return new ts(u, c, m, h, g);
		}
	}, Ra = class {
		constructor(e, t, n) {
		...
		computeDiff() {
			...
			for (let l = -1, o = t.length; l < o; l++) {
				...
				for (; s < c && a < m;) {
					const h = this.originalLines[s], g = this.modifiedLines[a];
					if (h !== g) {
						{
							let f = m1(h, 1), b = m1(g, 1);
							for (; f > 1 && b > 1 && h.charCodeAt(f - 2) === g.charCodeAt(b - 2);) f--, b--;
							(f > 1 || b > 1) && this._pushTrimWhitespaceCharChange(r, s + 1, 1, f, a + 1, 1, b);
						}
						{
							let f = f1(h, 1), b = f1(g, 1);
							const p = h.length + 1, y = g.length + 1;
	...
	var Be = class N1 {
		...
		static {
			this.zero = new Pt(0, 0);
		}
		static {
			this.max = new Pt(Number.MAX_SAFE_INTEGER, Number.MAX_SAFE_INTEGER);
		}
		constructor(t, n) {
...
`);
		...
		compute(e, t, n = g1.instance, r) {
			...
			for (let f = 0; f < e.length; f++) for (let b = 0; b < t.length; b++) {
				...
				const N = Math.max(p, y, v);
				if (N === v) {
					const _ = f > 0 && b > 0 ? l.get(f - 1, b - 1) : 0;
					l.set(f, b, _ + 1), a.set(f, b, 3);
				} else N === p ? (l.set(f, b, 0), a.set(f, b, 1)) : N === y && (l.set(f, b, 0), a.set(f, b, 2));
				s.set(f, b, N);
			}
			const o = [];
			...
			let h = e.length - 1, g = t.length - 1;
			for (; h >= 0 && g >= 0;) a.get(h, g) === 3 ? (m(h, g), h--, g--) : a.get(h, g) === 1 ? h-- : g--;
		...
		compute(e, t, n = g1.instance) {
			...
			e: for (;;) {
				...
				for (c = b; c <= p; c += 2) {
					const y = c === p ? -1 : o.get(c + 1), v = c === b ? -1 : o.get(c - 1) + 1, N = Math.min(Math.max(y, v), r.length), _ = N - c;
					if (N > r.length || _ > s.length) continue;
					const M = a(N, _);
					o.set(c, M);
					const F = N === y ? u.get(c + 1) : u.get(c - 1);
					if (u.set(c, M !== N ? new xr(F, N, _, M - N) : F), o.get(c) === r.length && o.get(c) - c === s.length) break e;
				}
			}
			let m = u.get(c);
			const h = [];
			let g = r.length, f = s.length;
			for (;;) {
				const b = m ? m.x + m.length : 0, p = m ? m.y + m.length : 0;
				if ((b !== g || p !== f) && h.push(new re(new $(b, g), new $(p, f))), !m) break;
				g = m.x, f = m.y, m = m.prev;
			}
		...
		extendToFullLines(e) {
			return new $(Te(this.firstElementOffsetByLineIdx, (r) => r <= e.start) ?? 0, ga(this.firstElementOffsetByLineIdx, (r) => e.endExclusive <= r) ?? this.elements.length);
		}
	};
	function qe(e) {
	...
	function Ea(e, t, n, r, s, a) {
		let { moves: l, excludedChanges: o } = Da(e, t, n, a);
		if (!a.isValid()) return [];
		const u = Ta(e.filter((c) => !o.has(c)), r, s, t, n, a);
		return ma(l, u), l = Fa(l), l = l.filter((c) => {
			const m = c.original.toOffsetRange().slice(t).map((h) => h.trim());
			return m.join(`
`).length >= 15 && Pa(m, (h) => h.length >= 2) >= 2;
		}), l = Ia(e, l), l;
	}
	function Pa(e, t) {
		let n = 0;
	...
	function Ta(e, t, n, r, s, a) {
		const l = [], o = new Yi();
		for (const g of e) for (let f = g.original.startLineNumber; f < g.original.endLineNumberExclusive - 2; f++) {
			const b = `${t[f - 1]}:${t[f + 1 - 1]}:${t[f + 2 - 1]}`;
			o.add(b, { range: new U(f, f + 3) });
		}
		const u = [];
		e.sort(je((g) => g.modified.startLineNumber, Xe));
		for (const g of e) {
			let f = [];
			for (let b = g.modified.startLineNumber; b < g.modified.endLineNumberExclusive - 2; b++) {
				const p = `${n[b - 1]}:${n[b + 1 - 1]}:${n[b + 2 - 1]}`, y = new U(b, b + 3), v = [];
				o.forEach(p, ({ range: N }) => {
					for (const M of f) if (M.originalLineRange.endLineNumberExclusive + 1 === N.endLineNumberExclusive && M.modifiedLineRange.endLineNumberExclusive + 1 === y.endLineNumberExclusive) {
						M.originalLineRange = new U(M.originalLineRange.startLineNumber, N.endLineNumberExclusive), M.modifiedLineRange = new U(M.modifiedLineRange.startLineNumber, y.endLineNumberExclusive), v.push(M);
						return;
					}
					const _ = {
						modifiedLineRange: y,
		...
		u.sort(fa(je((g) => g.modifiedLineRange.length, Xe)));
		const c = new wt(), m = new wt();
		for (const g of u) {
			const f = g.modifiedLineRange.startLineNumber - g.originalLineRange.startLineNumber, b = c.subtractFrom(g.modifiedLineRange), p = m.subtractFrom(g.originalLineRange).getWithDelta(f), y = b.getIntersection(p);
			for (const v of y.ranges) {
				if (v.length < 3) continue;
				const N = v, _ = v.delta(-f);
				l.push(new Ie(_, N)), c.addRange(N), m.addRange(_);
			}
		}
		l.sort(je((g) => g.original.startLineNumber, Xe));
		const h = new _r(e);
		for (let g = 0; g < l.length; g++) {
			const f = l[g], b = h.findLastMonotonous((k) => k.original.startLineNumber <= f.original.startLineNumber), p = Te(e, (k) => k.modified.startLineNumber <= f.modified.startLineNumber), y = Math.max(f.original.startLineNumber - b.original.startLineNumber, f.modified.startLineNumber - p.modified.startLineNumber), v = h.findLastMonotonous((k) => k.original.startLineNumber < f.original.endLineNumberExclusive), N = Te(e, (k) => k.modified.startLineNumber < f.modified.endLineNumberExclusive), _ = Math.max(v.original.endLineNumberExclusive - f.original.endLineNumberExclusive, N.modified.endLineNumberExclusive - f.modified.endLineNumberExclusive);
			let M;
			for (M = 0; M < y; M++) {
				const k = f.original.startLineNumber - M - 1, w = f.modified.startLineNumber - M - 1;
				if (k > r.length || w > s.length || c.contains(w) || m.contains(k) || !Pr(r[k - 1], s[w - 1], a)) break;
			}
			M > 0 && (m.addRange(new U(f.original.startLineNumber - M, f.original.startLineNumber)), c.addRange(new U(f.modified.startLineNumber - M, f.modified.startLineNumber)));
			let F;
			for (F = 0; F < _; F++) {
				const k = f.original.endLineNumberExclusive + F, w = f.modified.endLineNumberExclusive + F;
				if (k > r.length || w > s.length || c.contains(w) || m.contains(k) || !Pr(r[k - 1], s[w - 1], a)) break;
			}
			F > 0 && (m.addRange(new U(f.original.endLineNumberExclusive, f.original.endLineNumberExclusive + F)), c.addRange(new U(f.modified.endLineNumberExclusive, f.modified.endLineNumberExclusive + F))), (M > 0 || F > 0) && (l[g] = new Ie(new U(f.original.startLineNumber - M, f.original.endLineNumberExclusive + F), new U(f.modified.startLineNumber - M, f.modified.endLineNumberExclusive + F)));
		}
		return l;
	}
	function Pr(e, t, n) {
	...
	function Ia(e, t) {
		const n = new _r(e);
		return t = t.filter((r) => (n.findLastMonotonous((s) => s.original.startLineNumber < r.original.endLineNumberExclusive) || new Ie(new U(1, 1), new U(1, 1))) !== Te(e, (s) => s.modified.startLineNumber < r.modified.endLineNumberExclusive)), t;
	}
	function Dr(e, t, n) {
		let r = n;
		return r = Tr(e, t, r), r = Tr(e, t, r), r = Va(e, t, r), r;
	}
	function Tr(e, t, n) {
	...
	function Fr(e, t, n, r, s) {
		...
		let u = 0, c = -1;
		for (let m = -l; m <= o; m++) {
			const h = e.seq2Range.start + m, g = e.seq2Range.endExclusive + m, f = e.seq1Range.start + m, b = t.getBoundaryScore(f) + n.getBoundaryScore(h) + n.getBoundaryScore(g);
			b > c && (c = b, u = m);
		}
		return e.delta(u);
	}
	...
	function Ir(e, t, n, r, s = !1) {
		...
		function u(c, m) {
			...
			const h = r(e, c.offset1), g = r(t, c.offset2);
			if (!h || !g) return;
			let f = new re(h, g);
			...
			for (; a.length > 0;) {
				...
				const M = new re(r(e, v.seq1Range.start), r(t, v.seq2Range.start)), F = M.intersect(v);
				if (p += F.seq1Range.length, y += F.seq2Range.length, f = f.join(M), f.seq1Range.endExclusive >= v.seq1Range.endExclusive) a.shift();
				else break;
			}
		...
		do {
			...
			for (let u = 1; u < r.length; u++) {
				let h = function(g, f) {
					...
					return e.getText(b).replace(/\s/g, "").length <= 4 && (g.seq1Range.length + g.seq2Range.length > 5 || f.seq1Range.length + f.seq2Range.length > 5);
				};
		...
		do {
			...
			for (let c = 1; c < r.length; c++) {
				let g = function(f, b) {
					...
					const v = e.countLinesIn(f.seq1Range), N = f.seq1Range.length, _ = t.countLinesIn(f.seq2Range), M = f.seq2Range.length, F = e.countLinesIn(b.seq1Range), k = b.seq1Range.length, w = t.countLinesIn(b.seq2Range), L = b.seq2Range.length, x = 130;
					function q(C) {
						return Math.min(C, x);
					}
					return Math.pow(Math.pow(q(v * 40 + N), 1.5) + Math.pow(q(_ * 40 + M), 1.5), 1.5) + Math.pow(Math.pow(q(F * 40 + k), 1.5) + Math.pow(q(w * 40 + L), 1.5), 1.5) > (x ** 1.5) ** 1.5 * 1.3;
				};
				const m = r[c], h = u[u.length - 1];
				g(h, m) ? (a = !0, u[u.length - 1] = u[u.length - 1].join(m)) : u.push(m);
			}
			r = u;
		} while (s++ < 10 && a);
		const l = [];
...
`);
	...
	var Wa = class {
		constructor() {
			this.dynamicProgrammingDiffing = new Ca(), this.myersDiffingAlgorithm = new Cr();
		}
		computeDiff(e, t, n) {
			if (e.length <= 1 && oa(e, t, (k, w) => k === w)) return new bt([], [], !1);
			if (e.length === 1 && e[0].length === 0 || t.length === 1 && t[0].length === 0) return new bt([new Qe(new U(1, e.length + 1), new U(1, t.length + 1), [new de(new I(1, 1, e.length, e[e.length - 1].length + 1), new I(1, 1, t.length, t[t.length - 1].length + 1))])], [], !1);
			const r = n.maxComputationTimeMs === 0 ? g1.instance : new Aa(n.maxComputationTimeMs), s = !n.ignoreTrimWhitespace, a = /* @__PURE__ */ new Map();
			function l(k) {
				let w = a.get(k);
				return w === void 0 && (w = a.size, a.set(k, w)), w;
			}
			const o = e.map((k) => l(k.trim())), u = t.map((k) => l(k.trim())), c = new Vr(o, e), m = new Vr(u, t), h = c.length + m.length < 1700 ? this.dynamicProgrammingDiffing.compute(c, m, r, (k, w) => e[k] === t[w] ? t[w].length === 0 ? .1 : 1 + Math.log(1 + t[w].length) : .99) : this.myersDiffingAlgorithm.compute(c, m, r);
			let g = h.diffs, f = h.hitTimeout;
			g = Dr(c, m, g), g = Ua(c, m, g);
			const b = [], p = (k) => {
				if (s) for (let w = 0; w < k; w++) {
					const L = y + w, x = v + w;
					if (e[L] !== t[x]) {
						const q = this.refineDiff(e, t, new re(new $(L, L + 1), new $(x, x + 1)), r, s, n);
						for (const C of q.mappings) b.push(C);
						q.hitTimeout && (f = !0);
					}
				}
			};
			let y = 0, v = 0;
			for (const k of g) {
				st(() => k.seq1Range.start - y === k.seq2Range.start - v), p(k.seq1Range.start - y), y = k.seq1Range.endExclusive, v = k.seq2Range.endExclusive;
				const w = this.refineDiff(e, t, k, r, s, n);
				w.hitTimeout && (f = !0);
				for (const L of w.mappings) b.push(L);
			}
			p(e.length - y);
			const M = Lr(b, new _t(e), new _t(t));
			let F = [];
			return n.computeMoves && (F = this.computeMoves(M, e, t, o, u, r, s, n)), st(() => {
				function k(L, x) {
					if (L.lineNumber < 1 || L.lineNumber > x.length) return !1;
					const q = x[L.lineNumber - 1];
					return !(L.column < 1 || L.column > q.length + 1);
				}
				function w(L, x) {
				...
				for (const L of M) {
					...
					for (const x of L.innerChanges) if (!(k(x.modifiedRange.getStartPosition(), t) && k(x.modifiedRange.getEndPosition(), t) && k(x.originalRange.getStartPosition(), e) && k(x.originalRange.getEndPosition(), e))) return !1;
		...
		computeMoves(e, t, n, r, s, a, l, o) {
			return Ea(e, t, n, r, s, a).map((u) => {
				return new la(u, Lr(this.refineDiff(t, n, new re(u.original.toOffsetRange(), u.modified.toOffsetRange()), a, l, o).mappings, new _t(t), new _t(n), !0));
			});
		}
		refineDiff(e, t, n, r, s, a) {
			const l = $a(n).toRangeMapping2(e, t), o = new Lt(e, l.originalRange, s), u = new Lt(t, l.modifiedRange, s), c = o.length + u.length < 500 ? this.dynamicProgrammingDiffing.compute(o, u, r) : this.myersDiffingAlgorithm.compute(o, u, r);
			let m = c.diffs;
			return m = Dr(o, u, m), m = Ir(o, u, m, (h, g) => h.findWordContaining(g)), a.extendToSubwords && (m = Ir(o, u, m, (h, g) => h.findSubWordContaining(g), !0)), m = Ba(o, u, m), m = Ha(o, u, m), {
	...
	function $a(e) {
		return new Ie(new U(e.seq1Range.start + 1, e.seq1Range.endExclusive + 1), new U(e.seq2Range.start + 1, e.seq2Range.endExclusive + 1));
	}
	const qr = {
		getLegacy: () => new La(),
		getDefault: () => new Wa()
	...
	var d = class {
		...
		static toRGBA(t) {
			...
			return new d(Math.round(l * 255), Math.round(o * 255), Math.round(u * 255), a);
		}
	}, Nt = class rs {
		constructor(t, n, r, s) {
		...
		static fromRGBA(t) {
			...
			let u;
			return l === 0 ? u = 0 : a === n ? u = ((r - s) / l % 6 + 6) % 6 : a === r ? u = (s - n) / l + 2 : u = (n - r) / l + 4, new rs(Math.round(u * 60), o, a, t.a);
		}
		static toRGBA(t) {
			...
			return n < 60 ? (c = l, m = o) : n < 120 ? (c = o, m = l) : n < 180 ? (m = l, h = o) : n < 240 ? (m = o, h = l) : n < 300 ? (c = o, h = l) : n <= 360 && (c = l, h = o), c = Math.round((c + u) * 255), m = Math.round((m + u) * 255), h = Math.round((h + u) * 255), new d(c, m, h, a);
		}
	}, St = class X {
		static fromHex(t) {
		...
		constructor(t) {
			if (t) if (t instanceof d) this.rgba = t;
		...
		equals(t) {
			return !!t && d.equals(this.rgba, t.rgba) && Re.equals(this.hsla, t.hsla) && Nt.equals(this.hsva, t.hsva);
		}
		...
		opposite() {
			return new X(new d(255 - this.rgba.r, 255 - this.rgba.g, 255 - this.rgba.b, this.rgba.a));
		}
		mix(t, n = .5) {
			const r = Math.min(Math.max(n, 0), 1), s = this.rgba, a = t.rgba, l = s.r + (a.r - s.r) * r, o = s.g + (a.g - s.g) * r, u = s.b + (a.b - s.b) * r, c = s.a + (a.a - s.a) * r;
			return new X(new d(l, o, u, c));
		}
		makeOpaque(t) {
			if (this.isOpaque() || t.rgba.a !== 1) return this;
			const { r: n, g: r, b: s, a } = this.rgba;
			return new X(new d(t.rgba.r - a * (t.rgba.r - n), t.rgba.g - a * (t.rgba.g - r), t.rgba.b - a * (t.rgba.b - s), 1));
		}
		toString() {
		...
		static {
			this.white = new X(new d(255, 255, 255, 1));
		}
		static {
			this.black = new X(new d(0, 0, 0, 1));
		}
		static {
			this.red = new X(new d(255, 0, 0, 1));
		}
		static {
			this.blue = new X(new d(0, 0, 255, 1));
		}
		static {
			this.green = new X(new d(0, 255, 0, 1));
		}
		static {
			this.cyan = new X(new d(0, 255, 255, 1));
		}
		static {
			this.lightgrey = new X(new d(211, 211, 211, 1));
		}
		static {
			this.transparent = new X(new d(0, 0, 0, 0));
		}
	};
	(function(e) {
		(function(t) {
			(function(n) {
				...
				function h(p) {
					...
					if (p.startsWith("rgba(")) {
						...
						return new e(new d(parseInt(y.groups?.r ?? "0"), parseInt(y.groups?.g ?? "0"), parseInt(y.groups?.b ?? "0"), parseFloat(y.groups?.a ?? "0")));
					}
					if (p.startsWith("rgb(")) {
						const y = p.match(/rgb\((?<r>(?:\+|-)?\d+), *(?<g>(?:\+|-)?\d+), *(?<b>(?:\+|-)?\d+)\)/);
						if (!y) throw new Error("Invalid color format " + p);
						return new e(new d(parseInt(y.groups?.r ?? "0"), parseInt(y.groups?.g ?? "0"), parseInt(y.groups?.b ?? "0")));
					}
					return g(p);
				}
				n.parse = h;
				function g(p) {
					switch (p) {
						case "aliceblue": return new e(new d(240, 248, 255, 1));
						case "antiquewhite": return new e(new d(250, 235, 215, 1));
						case "aqua": return new e(new d(0, 255, 255, 1));
						case "aquamarine": return new e(new d(127, 255, 212, 1));
						case "azure": return new e(new d(240, 255, 255, 1));
						case "beige": return new e(new d(245, 245, 220, 1));
						case "bisque": return new e(new d(255, 228, 196, 1));
						case "black": return new e(new d(0, 0, 0, 1));
						case "blanchedalmond": return new e(new d(255, 235, 205, 1));
						case "blue": return new e(new d(0, 0, 255, 1));
						case "blueviolet": return new e(new d(138, 43, 226, 1));
						case "brown": return new e(new d(165, 42, 42, 1));
						case "burlywood": return new e(new d(222, 184, 135, 1));
						case "cadetblue": return new e(new d(95, 158, 160, 1));
						case "chartreuse": return new e(new d(127, 255, 0, 1));
						case "chocolate": return new e(new d(210, 105, 30, 1));
						case "coral": return new e(new d(255, 127, 80, 1));
						case "cornflowerblue": return new e(new d(100, 149, 237, 1));
						case "cornsilk": return new e(new d(255, 248, 220, 1));
						case "crimson": return new e(new d(220, 20, 60, 1));
						case "cyan": return new e(new d(0, 255, 255, 1));
						case "darkblue": return new e(new d(0, 0, 139, 1));
						case "darkcyan": return new e(new d(0, 139, 139, 1));
						case "darkgoldenrod": return new e(new d(184, 134, 11, 1));
						case "darkgray": return new e(new d(169, 169, 169, 1));
						case "darkgreen": return new e(new d(0, 100, 0, 1));
						case "darkgrey": return new e(new d(169, 169, 169, 1));
						case "darkkhaki": return new e(new d(189, 183, 107, 1));
						case "darkmagenta": return new e(new d(139, 0, 139, 1));
						case "darkolivegreen": return new e(new d(85, 107, 47, 1));
						case "darkorange": return new e(new d(255, 140, 0, 1));
						case "darkorchid": return new e(new d(153, 50, 204, 1));
						case "darkred": return new e(new d(139, 0, 0, 1));
						case "darksalmon": return new e(new d(233, 150, 122, 1));
						case "darkseagreen": return new e(new d(143, 188, 143, 1));
						case "darkslateblue": return new e(new d(72, 61, 139, 1));
						case "darkslategray": return new e(new d(47, 79, 79, 1));
						case "darkslategrey": return new e(new d(47, 79, 79, 1));
						case "darkturquoise": return new e(new d(0, 206, 209, 1));
						case "darkviolet": return new e(new d(148, 0, 211, 1));
						case "deeppink": return new e(new d(255, 20, 147, 1));
						case "deepskyblue": return new e(new d(0, 191, 255, 1));
						case "dimgray": return new e(new d(105, 105, 105, 1));
						case "dimgrey": return new e(new d(105, 105, 105, 1));
						case "dodgerblue": return new e(new d(30, 144, 255, 1));
						case "firebrick": return new e(new d(178, 34, 34, 1));
						case "floralwhite": return new e(new d(255, 250, 240, 1));
						case "forestgreen": return new e(new d(34, 139, 34, 1));
						case "fuchsia": return new e(new d(255, 0, 255, 1));
						case "gainsboro": return new e(new d(220, 220, 220, 1));
						case "ghostwhite": return new e(new d(248, 248, 255, 1));
						case "gold": return new e(new d(255, 215, 0, 1));
						case "goldenrod": return new e(new d(218, 165, 32, 1));
						case "gray": return new e(new d(128, 128, 128, 1));
						case "green": return new e(new d(0, 128, 0, 1));
						case "greenyellow": return new e(new d(173, 255, 47, 1));
						case "grey": return new e(new d(128, 128, 128, 1));
						case "honeydew": return new e(new d(240, 255, 240, 1));
						case "hotpink": return new e(new d(255, 105, 180, 1));
						case "indianred": return new e(new d(205, 92, 92, 1));
						case "indigo": return new e(new d(75, 0, 130, 1));
						case "ivory": return new e(new d(255, 255, 240, 1));
						case "khaki": return new e(new d(240, 230, 140, 1));
						case "lavender": return new e(new d(230, 230, 250, 1));
						case "lavenderblush": return new e(new d(255, 240, 245, 1));
						case "lawngreen": return new e(new d(124, 252, 0, 1));
						case "lemonchiffon": return new e(new d(255, 250, 205, 1));
						case "lightblue": return new e(new d(173, 216, 230, 1));
						case "lightcoral": return new e(new d(240, 128, 128, 1));
						case "lightcyan": return new e(new d(224, 255, 255, 1));
						case "lightgoldenrodyellow": return new e(new d(250, 250, 210, 1));
						case "lightgray": return new e(new d(211, 211, 211, 1));
						case "lightgreen": return new e(new d(144, 238, 144, 1));
						case "lightgrey": return new e(new d(211, 211, 211, 1));
						case "lightpink": return new e(new d(255, 182, 193, 1));
						case "lightsalmon": return new e(new d(255, 160, 122, 1));
						case "lightseagreen": return new e(new d(32, 178, 170, 1));
						case "lightskyblue": return new e(new d(135, 206, 250, 1));
						case "lightslategray": return new e(new d(119, 136, 153, 1));
						case "lightslategrey": return new e(new d(119, 136, 153, 1));
						case "lightsteelblue": return new e(new d(176, 196, 222, 1));
						case "lightyellow": return new e(new d(255, 255, 224, 1));
						case "lime": return new e(new d(0, 255, 0, 1));
						case "limegreen": return new e(new d(50, 205, 50, 1));
						case "linen": return new e(new d(250, 240, 230, 1));
						case "magenta": return new e(new d(255, 0, 255, 1));
						case "maroon": return new e(new d(128, 0, 0, 1));
						case "mediumaquamarine": return new e(new d(102, 205, 170, 1));
						case "mediumblue": return new e(new d(0, 0, 205, 1));
						case "mediumorchid": return new e(new d(186, 85, 211, 1));
						case "mediumpurple": return new e(new d(147, 112, 219, 1));
						case "mediumseagreen": return new e(new d(60, 179, 113, 1));
						case "mediumslateblue": return new e(new d(123, 104, 238, 1));
						case "mediumspringgreen": return new e(new d(0, 250, 154, 1));
						case "mediumturquoise": return new e(new d(72, 209, 204, 1));
						case "mediumvioletred": return new e(new d(199, 21, 133, 1));
						case "midnightblue": return new e(new d(25, 25, 112, 1));
						case "mintcream": return new e(new d(245, 255, 250, 1));
						case "mistyrose": return new e(new d(255, 228, 225, 1));
						case "moccasin": return new e(new d(255, 228, 181, 1));
						case "navajowhite": return new e(new d(255, 222, 173, 1));
						case "navy": return new e(new d(0, 0, 128, 1));
						case "oldlace": return new e(new d(253, 245, 230, 1));
						case "olive": return new e(new d(128, 128, 0, 1));
						case "olivedrab": return new e(new d(107, 142, 35, 1));
						case "orange": return new e(new d(255, 165, 0, 1));
						case "orangered": return new e(new d(255, 69, 0, 1));
						case "orchid": return new e(new d(218, 112, 214, 1));
						case "palegoldenrod": return new e(new d(238, 232, 170, 1));
						case "palegreen": return new e(new d(152, 251, 152, 1));
						case "paleturquoise": return new e(new d(175, 238, 238, 1));
						case "palevioletred": return new e(new d(219, 112, 147, 1));
						case "papayawhip": return new e(new d(255, 239, 213, 1));
						case "peachpuff": return new e(new d(255, 218, 185, 1));
						case "peru": return new e(new d(205, 133, 63, 1));
						case "pink": return new e(new d(255, 192, 203, 1));
						case "plum": return new e(new d(221, 160, 221, 1));
						case "powderblue": return new e(new d(176, 224, 230, 1));
						case "purple": return new e(new d(128, 0, 128, 1));
						case "rebeccapurple": return new e(new d(102, 51, 153, 1));
						case "red": return new e(new d(255, 0, 0, 1));
						case "rosybrown": return new e(new d(188, 143, 143, 1));
						case "royalblue": return new e(new d(65, 105, 225, 1));
						case "saddlebrown": return new e(new d(139, 69, 19, 1));
						case "salmon": return new e(new d(250, 128, 114, 1));
						case "sandybrown": return new e(new d(244, 164, 96, 1));
						case "seagreen": return new e(new d(46, 139, 87, 1));
						case "seashell": return new e(new d(255, 245, 238, 1));
						case "sienna": return new e(new d(160, 82, 45, 1));
						case "silver": return new e(new d(192, 192, 192, 1));
						case "skyblue": return new e(new d(135, 206, 235, 1));
						case "slateblue": return new e(new d(106, 90, 205, 1));
						case "slategray": return new e(new d(112, 128, 144, 1));
						case "slategrey": return new e(new d(112, 128, 144, 1));
						case "snow": return new e(new d(255, 250, 250, 1));
						case "springgreen": return new e(new d(0, 255, 127, 1));
						case "steelblue": return new e(new d(70, 130, 180, 1));
						case "tan": return new e(new d(210, 180, 140, 1));
						case "teal": return new e(new d(0, 128, 128, 1));
						case "thistle": return new e(new d(216, 191, 216, 1));
						case "tomato": return new e(new d(255, 99, 71, 1));
						case "turquoise": return new e(new d(64, 224, 208, 1));
						case "violet": return new e(new d(238, 130, 238, 1));
						case "wheat": return new e(new d(245, 222, 179, 1));
						case "white": return new e(new d(255, 255, 255, 1));
						case "whitesmoke": return new e(new d(245, 245, 245, 1));
						case "yellow": return new e(new d(255, 255, 0, 1));
						case "yellowgreen": return new e(new d(154, 205, 50, 1));
						default: return null;
					}
				}
				function f(p) {
					...
					if (y === 7) return new e(new d(16 * b(p.charCodeAt(1)) + b(p.charCodeAt(2)), 16 * b(p.charCodeAt(3)) + b(p.charCodeAt(4)), 16 * b(p.charCodeAt(5)) + b(p.charCodeAt(6)), 1));
					if (y === 9) return new e(new d(16 * b(p.charCodeAt(1)) + b(p.charCodeAt(2)), 16 * b(p.charCodeAt(3)) + b(p.charCodeAt(4)), 16 * b(p.charCodeAt(5)) + b(p.charCodeAt(6)), (16 * b(p.charCodeAt(7)) + b(p.charCodeAt(8))) / 255));
					if (y === 4) {
						const v = b(p.charCodeAt(1)), N = b(p.charCodeAt(2)), _ = b(p.charCodeAt(3));
						return new e(new d(16 * v + v, 16 * N + N, 16 * _ + _));
					}
					if (y === 5) {
						const v = b(p.charCodeAt(1)), N = b(p.charCodeAt(2)), _ = b(p.charCodeAt(3)), M = b(p.charCodeAt(4));
						return new e(new d(16 * v + v, 16 * N + N, 16 * _ + _, (16 * M + M) / 255));
					}
					return null;
				}
				n.parseHex = f;
	...
	function Oa(e) {
		const t = [], n = Je(e, /\b(rgb|rgba|hsl|hsla)(\([0-9\s,.\%]*\))|^(#)([A-Fa-f0-9]{3})\b|^(#)([A-Fa-f0-9]{4})\b|^(#)([A-Fa-f0-9]{6})\b|^(#)([A-Fa-f0-9]{8})\b|(?<=['"\s])(#)([A-Fa-f0-9]{3})\b|(?<=['"\s])(#)([A-Fa-f0-9]{4})\b|(?<=['"\s])(#)([A-Fa-f0-9]{6})\b|(?<=['"\s])(#)([A-Fa-f0-9]{8})\b/gm);
		if (n.length > 0) for (const r of n) {
			const s = r.filter((u) => u !== void 0), a = s[1], l = s[2];
			if (!l) continue;
			let o;
			a === "rgb" ? o = Hr(Ye(e, r), Je(l, /^\(\s*(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9][0-9]|[0-9])\s*,\s*(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9][0-9]|[0-9])\s*,\s*(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9][0-9]|[0-9])\s*\)$/gm), !1) : a === "rgba" ? o = Hr(Ye(e, r), Je(l, /^\(\s*(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9][0-9]|[0-9])\s*,\s*(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9][0-9]|[0-9])\s*,\s*(25[0-5]|2[0-4][0-9]|1[0-9]{2}|[1-9][0-9]|[0-9])\s*,\s*(0[.][0-9]+|[.][0-9]+|[01][.]|[01])\s*\)$/gm), !0) : a === "hsl" ? o = Wr(Ye(e, r), Je(l, /^\(\s*((?:360(?:\.0+)?|(?:36[0]|3[0-5][0-9]|[12][0-9][0-9]|[1-9]?[0-9])(?:\.\d+)?))\s*[\s,]\s*(100|\d{1,2}[.]\d*|\d{1,2})%\s*[\s,]\s*(100|\d{1,2}[.]\d*|\d{1,2})%\s*\)$/gm), !1) : a === "hsla" ? o = Wr(Ye(e, r), Je(l, /^\(\s*((?:360(?:\.0+)?|(?:36[0]|3[0-5][0-9]|[12][0-9][0-9]|[1-9]?[0-9])(?:\.\d+)?))\s*[\s,]\s*(100|\d{1,2}[.]\d*|\d{1,2})%\s*[\s,]\s*(100|\d{1,2}[.]\d*|\d{1,2})%\s*[\s,]\s*(0[.][0-9]+|[.][0-9]+|[01][.]0*|[01])\s*\)$/gm), !0) : a === "#" && (o = za(Ye(e, r), a + l)), o && t.push(o);
		}
		return t;
	}
	function Ga(e) {
		return !e || typeof e.getValue != "function" || typeof e.positionAt != "function" ? [] : Oa(e);
	}
	const ja = /^-+|-+$/g, Xa = 100;
	function Qa(e, t) {
		let n = [];
	...
	function Ja(e, t) {
		...
		const s = Ji(t.markSectionHeaderRegex), a = new RegExp(t.markSectionHeaderRegex, `gdm${s ? "s" : ""}`);
		if (Bs(a)) return n;
		for (let l = 1; l <= r; l += 95) {
			const o = Math.min(l + Xa - 1, r), u = [];
...
`);
			...
			let m;
			for (; (m = a.exec(c)) !== null;) {
				const h = c.substring(0, m.index), g = (h.match(/\n/g) || []).length, f = l + g, b = m[0].split(`
`), p = b.length, y = f + p - 1, v = h.lastIndexOf(`
`) + 1, N = m.index - v + 1, _ = b[b.length - 1], M = {
					range: {
						startLineNumber: f,
						startColumn: N,
						endLineNumber: y,
						endColumn: p === 1 ? N + m[0].length : _.length + 1
					},
					text: (m.groups ?? {}).label ?? "",
					hasSeparatorLine: ((m.groups ?? {}).separator ?? "") !== "",
					shouldBeInComments: !0
				};
				(M.text || M.hasSeparatorLine) && (n.length === 0 || n[n.length - 1].range.endLineNumber < M.range.startLineNumber) && n.push(M), a.lastIndex = m.index + m[0].length;
			}
		}
		return n;
	}
	...
	(class ae {
		constructor(t, n) {
			this._onReturn = n, this._producerConsumer = new t0(), this._iterator = {
	...
	var n0 = class {
		...
		$acceptNewModel(e) {
			this._models[e.url] = new a0(e1.parse(e.url), e.lines, e.EOL, e.versionId);
		}
		...
		constructor(t = null) {
			this._foreignModule = t, this._requestHandlerBrand = void 0, this._workerTextModelSyncServer = new i0();
		}
		dispose() {}
		async $ping() {
			return "pong";
		...
		async $computeUnicodeHighlights(t, n, r) {
			...
			return s ? ia.computeUnicodeHighlights(s, n, r) : {
				ranges: [],
				hasMore: !1,
		...
		async $computeMoreMinimalEdits(t, n, r) {
			...
			for (let { range: u, text: c, eol: m } of n) {
				...
				const g = ii(h, c, r), f = s.offsetAt(I.lift(u).getStartPosition());
				for (const b of g) {
		...
		async $navigateValueSet(t, n, r, s, a) {
			...
			return fi.INSTANCE.navigateValueSet(n, u, c, m, r);
		}
		$fmr(t, n) {
...
//# sourceMappingURL=editor.worker-D1euiQ9O.js.map