...
` + e.stack) : e;
	...
	(function(e) {
		...
		e.slice = y;
		function k(w, L = Number.POSITIVE_INFINITY) {
			const x = [];
			if (L === 0) return [x, w];
	...
	(function(e) {
		e.None = () => Ke.None;
		function t(M, S, A) {
			return f(M, () => {}, 0, void 0, S ?? !0, void 0, A);
		}
		e.defer = t;
		function n(M) {
			return (S, A = null, E) => {
				let T = !1, F;
				return F = M((U) => {
					if (!T) return F ? F.dispose() : T = !0, S.call(A, U);
				}, null, E), T && F.dispose(), F;
			};
		}
		e.once = n;
		function r(M, S) {
		...
		e.onceIf = r;
		function s(M, S, A) {
			return h((E, T = null, F) => M((U) => E.call(T, S(U)), null, F), A);
		}
		e.map = s;
		function a(M, S, A) {
			return h((E, T = null, F) => M((U) => {
				S(U), E.call(T, U);
			}, null, F), A);
		}
		e.forEach = a;
		function o(M, S, A) {
			return h((E, T = null, F) => M((U) => S(U) && E.call(T, U), null, F), A);
		}
		e.filter = o;
		function l(M) {
			return M;
		...
		e.signal = l;
		function u(...M) {
			return (S, A = null, E) => m(Os(...M.map((T) => T((F) => S.call(A, F)))), E);
		}
		e.any = u;
		function c(M, S, A, E) {
			let T = A;
			return s(M, (F) => (T = S(T, F), T), E);
		}
		e.reduce = c;
		function h(M, S) {
			let A;
			...
			return S?.add(E), E.event;
		}
		function m(M, S) {
			return S instanceof Array ? S.push(M) : S && S.add(M), M;
		}
		function f(M, S, A = 100, E = !1, T = !1, F, U) {
			let K, O, re, ee = 0, Z;
			const Ge = new he({
				leakWarningThreshold: F,
				onWillAddFirstListener() {
					K = M((nl) => {
						ee++, O = S(O, nl), E && !re && (Ge.fire(O), O = void 0), Z = () => {
							const rl = O;
							O = void 0, re = void 0, (!E || ee > 1) && Ge.fire(rl), ee = 0;
						}, typeof A == "number" ? (re && clearTimeout(re), re = setTimeout(Z, A)) : re === void 0 && (re = null, queueMicrotask(Z));
					});
				},
		...
		e.debounce = f;
		function d(M, S = 0, A, E) {
			return e.debounce(M, (T, F) => T ? (T.push(F), T) : [F], S, void 0, A ?? !0, void 0, E);
		}
		e.accumulate = d;
		function b(M, S, A = 100, E = !0, T = !0, F, U) {
			let K, O, re, ee = 0;
			const Z = new he({
				leakWarningThreshold: F,
				onWillAddFirstListener() {
					K = M((Ge) => {
						ee++, O = S(O, Ge), re === void 0 && (E && (Z.fire(O), O = void 0, ee = 0), typeof A == "number" ? re = setTimeout(() => {
							T && ee > 0 && Z.fire(O), O = void 0, re = void 0, ee = 0;
						}, A) : (re = 0, queueMicrotask(() => {
							T && ee > 0 && Z.fire(O), O = void 0, re = void 0, ee = 0;
						})));
					});
				},
		...
		e.latch = p;
		function v(M, S, A) {
			return [e.filter(M, S, A), e.filter(M, (E) => !S(E), A)];
		}
		e.split = v;
		function _(M, S, A = !1, E = [], T) {
			let F = E.slice(), U;
			r1() && (U = {
				stack: s1.create(),
				timerId: setTimeout(() => {
		...
		class k {
			...
			forEach(S) {
				return this.steps.push((A) => (S(A), A)), this;
			}
			filter(S) {
				return this.steps.push((A) => S(A) ? A : y), this;
			}
			reduce(S, A) {
				let E = A;
				return this.steps.push((T) => (E = S(E, T), E)), this;
			}
			latch(S = (A, E) => A === E) {
				let A = !0, E;
				return this.steps.push((T) => {
					const F = A || !S(T, E);
					return A = !1, E = T, F ? T : y;
				}), this;
			}
			evaluate(S) {
				for (const A of this.steps) if (S = A(S), S === y) break;
				return S;
			}
		}
		function B(M, S, A = (E) => E) {
			const E = (...K) => U.fire(A(...K)), T = () => M.on(S, E), F = () => M.removeListener(S, E), U = new he({
		...
		function C(M, S, A = (E) => E) {
			const E = (...K) => U.fire(A(...K)), T = () => M.addEventListener(S, E), F = () => M.removeEventListener(S, E), U = new he({
				onWillAddFirstListener: T,
				onDidRemoveLastListener: F
			});
			return U.event;
		...
		function w(M, S) {
			...
			return T.cancel = A, S && T.finally(() => a1(E, S)), T;
		}
		e.toPromise = w;
		function L(M, S) {
			return M((A) => S.fire(A));
		...
		class $ {
			constructor(S, A) {
				...
				this.emitter = new he(E), A && A.add(this.emitter);
			}
			beginUpdate(S) {
				this._counter++;
			}
		...
		function ue(M) {
			return (S, A, E) => {
				...
				const U = {
					...
					endUpdate() {
						T--, T === 0 && (M.reportChanges(), F && (F = !1, S.call(A)));
					},
...
`));
	...
	var Ie = class {
		...
		WALKTRACE(t, n, r, s, a, o, l, u, c, h, m, f, d, b, p, v, _, N) {
			let y = null, k = null, B = new b1(), C = n, w = r, L = d[0] - v[0] - s, x = -1073741824, $ = this.m_forwardHistory.length - 1;
			do {
				const R = L + t;
			...
			return this.ConcatenateChanges(y, k);
		}
		ComputeRecursionPoint(t, n, r, s, a, o, l) {
			...
			const b = n - t + (s - r), p = b + 1, v = new Int32Array(p), _ = new Int32Array(p), N = s - r, y = n - t, k = t - r, B = n - s, C = (y - N) % 2 === 0;
			v[N] = t, _[y] = n, l[0] = !1;
			for (let w = 1; w <= b / 2 + 1; w++) {
				let L = 0, x = 0;
				h = this.ClipDiagonalBound(N - w, w, N, p), m = this.ClipDiagonalBound(N + w, w, N, p);
				for (let R = h; R <= m; R += 2) {
					R === h || R < m && v[R - 1] < v[R + 1] ? u = v[R + 1] : u = v[R - 1] + 1, c = u - (R - N) - k;
					const ue = u;
					for (; u < n && c < s && this.ElementsAreEqual(u + 1, c + 1);) u++, c++;
					if (v[R] = u, u + c > L + x && (L = u, x = c), !C && Math.abs(R - y) <= w - 1 && u >= _[R]) return a[0] = u, o[0] = c, ue <= _[R] && w <= 1448 ? this.WALKTRACE(N, h, m, k, y, f, d, B, v, _, u, n, a, c, s, o, C, l) : null;
				}
				const $ = (L - t + (x - r) - w) / 2;
				if (this.ContinueProcessingPredicate !== null && !this.ContinueProcessingPredicate(L, $)) return l[0] = !0, a[0] = L, o[0] = x, $ > 0 && w <= 1448 ? this.WALKTRACE(N, h, m, k, y, f, d, B, v, _, u, n, a, c, s, o, C, l) : (t++, r++, [new Se(t, n - t + 1, r, s - r + 1)]);
				...
				for (let R = f; R <= d; R += 2) {
					...
					if (_[R] = u, C && Math.abs(R - N) <= w && u <= v[R]) return a[0] = u, o[0] = c, ue >= v[R] && w <= 1448 ? this.WALKTRACE(N, h, m, k, y, f, d, B, v, _, u, n, a, c, s, o, C, l) : null;
				}
				if (w <= 1447) {
			...
			return this.WALKTRACE(N, h, m, k, y, f, d, B, v, _, u, n, a, c, s, o, C, l);
		}
		PrettifyChanges(t) {
	...
	const te = {
		...
		relative(e, t) {
			...
			if (n.length !== e.length || r.length !== t.length) {
				...
				let y = 0;
				for (; y < N && b[y].toLowerCase() === p[y].toLowerCase(); y++);
	...
	var Ba = class {
		static computeUnicodeHighlights(e, t, n) {
			...
			e: for (let p = r, v = s; p <= v; p++) {
				...
				do
					if (m = u.next(_), m) {
						let y = m.index, k = m.index + m[0].length;
						if (y > 0) _t(_.charCodeAt(y - 1)) && y--;
						if (k + 1 < N) _t(_.charCodeAt(k - 1)) && k++;
						const B = _.substring(y, k);
						let C = Ln(y + 1, Wr, _, 0);
		...
		compute(e, t, n = En.instance) {
			...
			e: for (;;) {
				...
				for (c = b; c <= p; c += 2) {
					...
					if (u.set(c, k !== N ? new rs(B, N, y, k - N) : B), l.get(c) === r.length && l.get(c) - c === s.length) break e;
				}
			}
			let h = u.get(c);
			const m = [];
	...
	function co(e, t, n, r, s, a) {
		...
		for (const f of e) {
			...
			for (let b = f.modified.startLineNumber; b < f.modified.endLineNumberExclusive - 2; b++) {
				...
				l.forEach(p, ({ range: N }) => {
					for (const k of d) if (k.originalLineRange.endLineNumberExclusive + 1 === N.endLineNumberExclusive && k.modifiedLineRange.endLineNumberExclusive + 1 === v.endLineNumberExclusive) {
						k.originalLineRange = new H(k.originalLineRange.startLineNumber, N.endLineNumberExclusive), k.modifiedLineRange = new H(k.modifiedLineRange.startLineNumber, v.endLineNumberExclusive), _.push(k);
						return;
					}
					const y = {
						modifiedLineRange: v,
		...
		for (let f = 0; f < o.length; f++) {
			...
			let k = 0;
			for (; k < v; k++) {
				const C = d.original.startLineNumber - k - 1, w = d.modified.startLineNumber - k - 1;
				if (C > r.length || w > s.length || c.contains(w) || h.contains(C) || !os(r[C - 1], s[w - 1], a)) break;
			}
			k > 0 && (h.addRange(new H(d.original.startLineNumber - k, d.original.startLineNumber)), c.addRange(new H(d.modified.startLineNumber - k, d.modified.startLineNumber)));
			let B = 0;
			for (; B < y; B++) {
				const C = d.original.endLineNumberExclusive + B, w = d.modified.endLineNumberExclusive + B;
				if (C > r.length || w > s.length || c.contains(w) || h.contains(C) || !os(r[C - 1], s[w - 1], a)) break;
			}
			B > 0 && (h.addRange(new H(d.original.endLineNumberExclusive, d.original.endLineNumberExclusive + B)), c.addRange(new H(d.modified.endLineNumberExclusive, d.modified.endLineNumberExclusive + B))), (k > 0 || B > 0) && (o[f] = new ke(new H(d.original.startLineNumber - k, d.original.endLineNumberExclusive + B), new H(d.modified.startLineNumber - k, d.modified.endLineNumberExclusive + B)));
		}
		return o;
	}
	function os(e, t, n) {
	...
	function us(e, t, n) {
		...
		for (let a = 1; a < n.length; a++) {
			...
			if (l.seq1Range.isEmpty || l.seq2Range.isEmpty) {
				...
				let c = 1;
				for (; c <= u && !(e.getElement(l.seq1Range.start - c) !== e.getElement(l.seq1Range.endExclusive - c) || t.getElement(l.seq2Range.start - c) !== t.getElement(l.seq2Range.endExclusive - c)); c++);
				if (c--, c === u) {
		...
		for (let a = 0; a < r.length - 1; a++) {
			...
			if (l.seq1Range.isEmpty || l.seq2Range.isEmpty) {
				...
				let c = 0;
				for (; c < u && !(!e.isStronglyEqual(l.seq1Range.start + c, l.seq1Range.endExclusive + c) || !t.isStronglyEqual(l.seq2Range.start + c, l.seq2Range.endExclusive + c)); c++);
				if (c === u) {
	...
	function hs(e, t, n, r, s = !1) {
		...
		function u(c, h) {
			...
			for (; a.length > 0;) {
				...
				const k = new ae(r(e, _.seq1Range.start), r(t, _.seq2Range.start)), B = k.intersect(_);
				if (p += B.seq1Range.length, v += B.seq2Range.length, d = d.join(k), d.seq1Range.endExclusive >= _.seq1Range.endExclusive) a.shift();
				else break;
			}
		...
		do {
			...
			for (let c = 1; c < r.length; c++) {
				let f = function(d, b) {
					...
					const _ = e.countLinesIn(d.seq1Range), N = d.seq1Range.length, y = t.countLinesIn(d.seq2Range), k = d.seq2Range.length, B = e.countLinesIn(b.seq1Range), C = b.seq1Range.length, w = t.countLinesIn(b.seq2Range), L = b.seq2Range.length, x = 130;
					function $(R) {
						return Math.min(R, x);
					}
					return Math.pow(Math.pow($(_ * 40 + N), 1.5) + Math.pow($(y * 40 + k), 1.5), 1.5) + Math.pow(Math.pow($(B * 40 + C), 1.5) + Math.pow($(w * 40 + L), 1.5), 1.5) > (x ** 1.5) ** 1.5 * 1.3;
				};
...
`);
	...
	var vo = class {
		...
		computeDiff(e, t, n) {
			...
			p(e.length - v);
			const k = Rn(b, new At(e), new At(t));
			let B = [];
			return n.computeMoves && (B = this.computeMoves(k, e, t, l, u, r, s, n)), Je(() => {
				function C(L, x) {
		...
		(function(t) {
			(function(n) {
				...
				function d(p) {
					...
					if (v === 5) {
						const _ = b(p.charCodeAt(1)), N = b(p.charCodeAt(2)), y = b(p.charCodeAt(3)), k = b(p.charCodeAt(4));
						return new e(new g(16 * _ + _, 16 * N + N, 16 * y + y, (16 * k + k) / 255));
					}
					return null;
				}
				n.parseHex = d;
...
`) + 1, N = h.index - _ + 1, y = b[b.length - 1], k = {
					range: {
						startLineNumber: d,
						startColumn: N,
						endLineNumber: v,
				...
				(k.text || k.hasSeparatorLine) && (n.length === 0 || n[n.length - 1].range.endLineNumber < k.range.startLineNumber) && n.push(k), a.lastIndex = h.index + h[0].length;
			}
		}
		return n;
	}
...
//# sourceMappingURL=editor.worker-1-WJhkdp.js.map