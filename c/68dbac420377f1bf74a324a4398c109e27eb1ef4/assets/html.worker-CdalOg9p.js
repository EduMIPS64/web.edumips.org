...
` + e.stack) : e;
	...
	(function(e) {
		...
		function T(E, _, R = !1, M = [], I) {
			...
			I && I.add(B);
			const Q = () => {
				P?.forEach((te) => Z.fire(te)), P = null, q();
			}, Z = new _e({
				onWillAddFirstListener() {
		...
		function W(E, _, R = (M) => M) {
			const M = (...q) => D.fire(R(...q)), I = () => E.on(_, M), P = () => E.removeListener(_, M), D = new _e({
		...
		function w(E, _, R = (M) => M) {
			const M = (...q) => D.fire(R(...q)), I = () => E.addEventListener(_, M), P = () => E.removeEventListener(_, M), D = new _e({
				onWillAddFirstListener: I,
				onDidRemoveLastListener: P
			});
			return D.event;
...
`));
	...
	var Qe = class {
		...
		WALKTRACE(t, n, r, i, s, a, o, l, c, u, d, m, f, b, g, y, T, k) {
			let A = null, H = null, W = new wr(), w = n, p = r, v = f[0] - y[0] - i, C = -1073741824, z = this.m_forwardHistory.length - 1;
			do {
				const S = v + t;
				S === w || S < p && c[S - 1] < c[S + 1] ? (d = c[S + 1], b = d - v - i, d < C && W.MarkNextChange(), C = d, W.AddModifiedElement(d + 1, b), v = S + 1 - t) : (d = c[S - 1] + 1, b = d - v - i, d < C && W.MarkNextChange(), C = d - 1, W.AddOriginalElement(d, b + 1), v = S - 1 - t), z >= 0 && (c = this.m_forwardHistory[z], t = c[0], w = 1, p = c.length - 1);
			} while (--z >= -1);
			if (A = W.getReverseChanges(), k[0]) {
				let S = f[0] + 1, N = y[0] + 1;
				...
				W = new wr(), w = a, p = o, v = f[0] - y[0] - l, C = 1073741824, z = T ? this.m_reverseHistory.length - 1 : this.m_reverseHistory.length - 2;
				do {
					const S = v + s;
					S === w || S < p && u[S - 1] >= u[S + 1] ? (d = u[S + 1] - 1, b = d - v - l, d > C && W.MarkNextChange(), C = d + 1, W.AddOriginalElement(d + 1, b + 1), v = S + 1 - s) : (d = u[S - 1], b = d - v - l, d > C && W.MarkNextChange(), C = d, W.AddModifiedElement(d + 1, b + 1), v = S - 1 - s), z >= 0 && (u = this.m_reverseHistory[z], s = u[0], w = 1, p = u.length - 1);
				} while (--z >= -1);
				H = W.getChanges();
			}
		...
		ComputeRecursionPoint(t, n, r, i, s, a, o) {
			...
			const b = n - t + (i - r), g = b + 1, y = new Int32Array(g), T = new Int32Array(g), k = i - r, A = n - t, H = t - r, W = n - i, w = (A - k) % 2 === 0;
			y[k] = t, T[A] = n, o[0] = !1;
			for (let p = 1; p <= b / 2 + 1; p++) {
				...
				for (let S = u; S <= d; S += 2) {
					...
					if (y[S] = l, l + c > v + C && (v = l, C = c), !w && Math.abs(S - A) <= p - 1 && l >= T[S]) return s[0] = l, a[0] = c, N <= T[S] && p <= 1448 ? this.WALKTRACE(k, u, d, H, A, m, f, W, y, T, l, n, s, c, i, a, w, o) : null;
				}
				const z = (v - t + (C - r) - p) / 2;
				if (this.ContinueProcessingPredicate !== null && !this.ContinueProcessingPredicate(v, z)) return o[0] = !0, s[0] = v, a[0] = C, z > 0 && p <= 1448 ? this.WALKTRACE(k, u, d, H, A, m, f, W, y, T, l, n, s, c, i, a, w, o) : (t++, r++, [new De(t, n - t + 1, r, i - r + 1)]);
				m = this.ClipDiagonalBound(A - p, p, A, g), f = this.ClipDiagonalBound(A + p, p, A, g);
				for (let S = m; S <= f; S += 2) {
					S === m || S < f && T[S - 1] >= T[S + 1] ? l = T[S + 1] - 1 : l = T[S - 1], c = l - (S - A) - W;
					const N = l;
					for (; l > t && c > r && this.ElementsAreEqual(l, c);) l--, c--;
					if (T[S] = l, w && Math.abs(S - k) <= p && l <= y[S]) return s[0] = l, a[0] = c, N >= y[S] && p <= 1448 ? this.WALKTRACE(k, u, d, H, A, m, f, W, y, T, l, n, s, c, i, a, w, o) : null;
				}
				if (p <= 1447) {
			...
			return this.WALKTRACE(k, u, d, H, A, m, f, W, y, T, l, n, s, c, i, a, w, o);
		}
		PrettifyChanges(t) {
	...
	const ce = {
		...
		relative(e, t) {
			...
			if (n.length !== e.length || r.length !== t.length) {
				...
				let A;
				for (A = 0; A < k && b[A].toLowerCase() === g[A].toLowerCase(); A++);
			...
			e: for (let g = r, y = i; g <= y; g++) {
				...
				do
					if (d = l.next(T), d) {
						...
						const W = T.substring(A, H);
						let w = ti(A + 1, Bs, T, 0);
						w && w.endColumn <= A + 1 && (w = null);
						const p = s.shouldHighlightNonBasicASCII(W, w ? w.word : null);
						if (p !== 0) {
		...
		compute(e, t, n = li.instance) {
			...
			e: for (;;) {
				...
				for (c = b; c <= g; c += 2) {
					...
					const H = s(k, A);
					o.set(c, H);
					const W = k === y ? l.get(c + 1) : l.get(c - 1);
					if (l.set(c, H !== k ? new ra(W, k, A, H - k) : W), o.get(c) === r.length && o.get(c) - c === i.length) break e;
				}
			}
			let u = l.get(c);
			const d = [];
...
`).length >= 15 && Sh(u, (d) => d.length >= 2) >= 2;
	...
	function xh(e, t, n, r, i, s) {
		...
		for (let m = 0; m < a.length; m++) {
			...
			let W;
			for (W = 0; W < A; W++) {
				const w = f.original.endLineNumberExclusive + W, p = f.modified.endLineNumberExclusive + W;
				if (w > r.length || p > i.length || c.contains(p) || u.contains(w) || !la(r[w - 1], i[p - 1], s)) break;
			}
			W > 0 && (u.addRange(new Y(f.original.endLineNumberExclusive, f.original.endLineNumberExclusive + W)), c.addRange(new Y(f.modified.endLineNumberExclusive, f.modified.endLineNumberExclusive + W))), (H > 0 || W > 0) && (a[m] = new Ve(new Y(f.original.startLineNumber - H, f.original.endLineNumberExclusive + W), new Y(f.modified.startLineNumber - H, f.modified.endLineNumberExclusive + W)));
		}
		return a;
	}
	function la(e, t, n) {
	...
	function da(e, t, n, r, i = !1) {
		...
		function l(c, u) {
			...
			for (; s.length > 0;) {
				...
				const H = new me(r(e, T.seq1Range.start), r(t, T.seq2Range.start)), W = H.intersect(T);
				if (g += W.seq1Range.length, y += W.seq2Range.length, f = f.join(H), f.seq1Range.endExclusive >= T.seq1Range.endExclusive) s.shift();
				else break;
			}
		...
		do {
			...
			for (let c = 1; c < r.length; c++) {
				let m = function(f, b) {
					...
					const T = e.countLinesIn(f.seq1Range), k = f.seq1Range.length, A = t.countLinesIn(f.seq2Range), H = f.seq2Range.length, W = e.countLinesIn(b.seq1Range), w = b.seq1Range.length, p = t.countLinesIn(b.seq2Range), v = b.seq2Range.length, C = 130;
					function z(S) {
						return Math.min(S, C);
					}
					return Math.pow(Math.pow(z(T * 40 + k), 1.5) + Math.pow(z(A * 40 + H), 1.5), 1.5) + Math.pow(Math.pow(z(W * 40 + w), 1.5) + Math.pow(z(p * 40 + v), 1.5), 1.5) > (C ** 1.5) ** 1.5 * 1.3;
				};
...
`);
	...
	var zh = class {
		...
		computeDiff(e, t, n) {
			...
			let W = [];
			return n.computeMoves && (W = this.computeMoves(H, e, t, o, l, r, i, n)), ft(() => {
				function w(v, C) {
		...
		function T() {
			...
			switch (s) {
				...
				case j.BeforeAttributeValue:
					...
					const W = i.peekChar();
					return W === Su || W === ku ? (i.advance(1), i.advanceUntilChar(W) && i.advance(1), d === "type" && (m = i.getSource().substring(k + 1, i.pos() - 1)), s = j.WithinTag, c = !1, g(k, U.AttributeValue)) : (s = j.WithinTag, c = !1, T());
...
\r`.indexOf(M) >= 0) return c.substring(R, _);
				...
				for (R && (D = D.parent); D;) {
					...
					if (q && (!D.closed || D.endTagStart && D.endTagStart > u)) {
						const B = {
							...
							kind: pe.Property,
							filterText: "/" + q,
							textEdit: le.replace(I, "/" + q + P),
							insertTextFormat: ke.PlainText
						}, Q = T(D.start), Z = T(_ - 1);
						if (Q !== null && Z !== null && Q !== Z) {
							const te = Q + "</" + q + P;
							B.textEdit = le.replace(g(_ - 1 - Z.length), te), B.filterText = Z + "</" + q;
						}
						return i.items.push(B), i;
					}
					D = D.parent;
				}
				return R || a.forEach((q) => {
			...
			function p(_, R) {
				...
				function P(D) {
					D.attributeNames.forEach((q) => {
						qe(q, M) && !I[q] && !R[q] && (I[q] = q + "=\"$1\"");
					}), D.children.forEach((q) => P(q));
				}
			...
			function v(_, R = u) {
				...
				if (u > _ && u <= R && Yu(c[_])) {
					const D = _ + 1;
					let q = R;
					R > _ && c[R - 1] === c[_] && q--, M = g(Ju(c, u, D), Qu(c, u, q)), P = u >= D && u <= q ? c.substring(D, u) : "", I = !1;
				} else M = g(_, R), P = c.substring(_, u), I = !0;
				if (s.length > 0) {
					const D = f.toLowerCase(), q = b.toLowerCase(), B = g(_, R);
				...
				return a.forEach((D) => {
					D.provideValues(f, b).forEach((q) => {
						const B = I ? "\"" + q.name + "\"" : q.name;
						i.items.push({
							label: q.name,
							filterText: B,
							kind: pe.Unit,
	...
	var Zu = class {
		...
		doHover(e, t, n, r) {
			...
			const W = b(U.AttributeValue, o.start);
			if (W) {
				const w = o.tag, p = Ku(e.getText(W)), v = H(o.start, e.offsetAt(W.start));
				if (v) return m(w, v, p, W);
			}
			return null;
		}
		convertContents(e) {
...
`);
					...
					for (var W = 0, w = !1, p = !1, v = !1, C = !1, z = !1, S = this._ch, N = !1, E, _, R; E = this._input.read(b), _ = E !== "", R = S, this._ch = this._input.next(), this._ch === "\\" && this._input.hasNext() && (this._ch += this._input.next()), S = this._ch, this._ch;) if (this._ch === "/" && this._input.peek() === "*") {
						...
						P.match(/[ :]$/) && (P = this.eatString(": ").replace(/\s+$/, ""), this.print_string(P), this._output.space_before_token = !0), W === 0 && P.indexOf(":") !== -1 && (p = !0, this.indent());
						...
						D.match(/[ :]$/) && (D = this.eatString(": ").replace(/\s+$/, ""), this.print_string(D), this._output.space_before_token = !0), W === 0 && D.indexOf(":") !== -1 ? (p = !0, this.indent()) : D in this.NESTED_AT_RULE ? (this._nestedLevel += 1, D in this.CONDITIONAL_GROUP_RULE && (v = !0)) : W === 0 && !p && (C = !0);
					}
						...
						(w || v) && !(this._input.lookBack("&") || this.foundNestedPseudoClass()) && !this._input.lookBack("(") && !C && W === 0 ? (this.print_string(":"), p || (p = !0, this._output.space_before_token = !0, this.eatWhitespace(!0), this.indent())) : (this._input.lookBack(" ") && (this._output.space_before_token = !0), this._input.peek() === ":" ? (this._ch = this._input.next(), this.print_string("::")) : this.print_string(":"));
					} else if (this._ch === "\"" || this._ch === "'") {
						var B = R === "\"" || R === "'";
						this.preserveSingleSpace(B || _), this.print_string(this._ch + this.eatString(this._ch)), this.eatWhitespace(!0);
					} else if (this._ch === ";") N = !1, W === 0 ? (p && (this.outdent(), p = !1), C = !1, this.print_string(this._ch), this.eatWhitespace(!0), this._input.peek() !== "/" && this._output.add_new_line()) : (this.print_string(this._ch), this.eatWhitespace(!0), this._output.space_before_token = !0);
					else if (this._ch === "(") if (this._input.lookBack("url")) this.print_string(this._ch), this.eatWhitespace(), W++, this.indent(), this._ch = this._input.next(), this._ch === ")" || this._ch === "\"" || this._ch === "'" ? this._input.back() : this._ch && (this.print_string(this._ch + this.eatString(")")), W && (W--, this.outdent()));
					else {
						var Q = !1;
						this._input.lookBack("with") && (Q = !0), this.preserveSingleSpace(_ || Q), this.print_string(this._ch), p && R === "$" && this._options.selector_separator_newline ? (this._output.add_new_line(), z = !0) : (this.eatWhitespace(), W++, this.indent());
					}
					else if (this._ch === ")") W && (W--, this.outdent()), z && this._input.peek() === ";" && this._options.selector_separator_newline && (z = !1, this.outdent(), this._output.add_new_line()), this.print_string(this._ch);
					else if (this._ch === ",") this.print_string(this._ch), this.eatWhitespace(!0), this._options.selector_separator_newline && (!p || z) && W === 0 && !C ? this._output.add_new_line() : this._output.space_before_token = !0;
					else if ((this._ch === ">" || this._ch === "+" || this._ch === "~") && !p && W === 0) this._options.space_around_combinator ? (this._output.space_before_token = !0, this.print_string(this._ch), this._output.space_before_token = !0) : (this.print_string(this._ch), this.eatWhitespace(), this._ch && f.test(this._ch) && (this._ch = ""));
...
`);
					...
					if (p.text !== "") {
						...
						v.custom_beautifier_name === "javascript" && typeof this._js_beautify == "function" ? S = this._js_beautify : v.custom_beautifier_name === "css" && typeof this._css_beautify == "function" ? S = this._css_beautify : v.custom_beautifier_name === "html" && (S = function(q, B) {
							return new k(q, B, C._js_beautify, C._css_beautify).beautify();
				...
				k.prototype._do_optional_end_element = function(w) {
					...
					if (!(w.is_empty_element || !w.is_start_tag || !w.parent)) {
						if (w.tag_name === "body") p = p || this._tag_stack.try_pop("head");
							...
							(!v || W.indexOf(v.tag_name) === -1) && (p = p || this._tag_stack.try_pop("p"));
		...
		(() => {
			...
			function A(S, N) {
				...
				if (R && (_ += R, _ += ":"), (M || R === "file") && (_ += u, _ += u), M) {
					let q = M.indexOf("@");
					if (q !== -1) {
						const B = M.substr(0, q);
						M = M.substr(q + 1), q = B.lastIndexOf(":"), q === -1 ? _ += E(B, !1, !1) : (_ += E(B.substr(0, q), !1, !1), _ += ":", _ += E(B.substr(q + 1), !1, !0)), _ += "@";
					}
					M = M.toLowerCase(), q = M.lastIndexOf(":"), q === -1 ? _ += E(M, !1, !0) : (_ += E(M.substr(0, q), !1, !0), _ += M.substr(q));
				}
				if (I) {
					if (I.length >= 3 && I.charCodeAt(0) === 47 && I.charCodeAt(2) === 58) {
						const q = I.charCodeAt(1);
						q >= 65 && q <= 90 && (I = `/${String.fromCharCode(q + 32)}:${I.substr(3)}`);
					} else if (I.length >= 2 && I.charCodeAt(1) === 58) {
						const q = I.charCodeAt(0);
						q >= 65 && q <= 90 && (I = `${String.fromCharCode(q + 32)}:${I.substr(2)}`);
					}
					_ += E(I, !0, !1);
				}
			...
			function w(S) {
				return S.match(W) ? S.replace(W, ((N) => H(N))) : S;
			}
			var p = n(470);
			const v = p.posix || p, C = "/";
			var z;
...
//# sourceMappingURL=html.worker-CdalOg9p.js.map